...
def scenario(self, func="mannWhitneyUTest({params})", table=None, snapshot_id=None):
    ...
    with Check("with group by"):
        execute_query(
            f"SELECT {func.format(params='number, number % 2')}, any(toTypeName(number)), any(toTypeName(number)) FROM numbers(100) GROUP BY number % 5"
        )

    with Check("some negative values"):
        execute_query(
            f"SELECT {func.format(params='number-5, number % 2')}, any(toTypeName(number)), any(toTypeName(number)) FROM numbers(1, 10)"
        )

    with Check("NULL value handling"):
        execute_query(
            f"SELECT {func.format(params='x,x % 2')}, any(toTypeName(x)), any(toTypeName(x)) FROM values('x Nullable(Int8)', 0, 1, NULL, 3, 4, 5)"
        )
    ...
    with Check("doc example"):
        execute_query(
            f"SELECT {func.format(params='sample_data,sample_index')}, any(toTypeName(sample_data)), any(toTypeName(sample_index)) FROM values('sample_data Int8, sample_index Int8', (10,0), (11,0), (12,0), (1,1), (2,1), (3,1))"
        )
    ...
    for v in ["inf", "-inf", "nan"]:
        with Check(f"{v}"):
            execute_query(
                f"SELECT {func.format(params='x,y')}, any(toTypeName(x)), any(toTypeName(y))  FROM values('x Float64, y Int8', (0,0), (2.3,1), ({v},0), (6.7,1), (4,0), (5,0))"
            )
    with Check(f"inf, -inf, nan"):
        execute_query(
            f"SELECT {func.format(params='x,y')}, any(toTypeName(x)), any(toTypeName(y))  FROM values('x Float64, y Int8', (nan,0), (2.3,1), (inf,1), (6.7,0), (-inf,0), (5,1))"
        )

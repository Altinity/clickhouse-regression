...
def scenario(self, func="sparkbar({params})", table=None):
    ...
    with Check("zero rows"):
        execute_query(
            f"SELECT {func.format(params='toUInt64(number), toUInt64(number+1)')} FROM numbers(0)"
        )

    with Check("single row"):
        execute_query(
            f"SELECT {func.format(params='toUInt64(number),toUInt64(number)')} FROM numbers(1)"
        )

    with Check("single row with zero weight"):
        execute_query(
            f"SELECT {func.format(params='toUInt64(number),toUInt64(number-1)')} FROM numbers(1)"
        )

    with Check("with group by"):
        execute_query(
            f"SELECT number % 2 AS even, {func.format(params='toUInt64(number), toUInt64(number)')} FROM numbers(10) GROUP BY even"
        )

    with Check("NULL value handling"):
        execute_query(
            f"SELECT {func.format(params='x,1')}  FROM values('x Nullable(UInt8)', 0, 1, NULL, 3, 4, 5)"
        )

    with Check("weight NULL value handling"):
        execute_query(
            f"SELECT {func.format(params='x,w')}  FROM values('x UInt8, w Nullable(UInt8)', (1,0), (1,1), (2,NULL), (3,3), (0,4), (1, 5))"
        )

    with Check("single NULL value"):
        execute_query(
            f"SELECT {func.format(params='x,w')}  FROM values('x Nullable(UInt8), w Nullable(UInt8)', (NULL,NULL) )"
        )

...
def scenario(self, func="rankCorr({params})", table=None, snapshot_id=None):
    ...
    with Check("with group by"):
        ...
        if (
            "State" in self.name
            and "_binary" in getattr(self.context, "clickhouse_path", "")
            and check_clickhouse_version("<26.6")(current())
        ):
            snapshot_name_override = current().name.replace("/part 3", "") + "_binary"
        execute_query(

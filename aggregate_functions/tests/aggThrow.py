...
@TestScenario
@Name("aggThrow")
@Requirements(RQ_SRS_031_ClickHouse_AggregateFunctions_Miscellaneous_AggThrow("1.0"))
def scenario(
    self,
    ...
    with Check("constant"):
        execute_query(
            f"SELECT {func.format(params='1')}", exitcode=exitcode, message=message
        )

    with Check("zero rows"):
        execute_query(
            f"SELECT {func.format(params='number')} FROM numbers(0)",
            exitcode=exitcode,
            message=message,
        )
    ...
    with Check("some negative values"):
        execute_query(
            f"SELECT {func.format(params='number-5')} FROM numbers(1, 10)",
            exitcode=exitcode,
            message=message,
        )

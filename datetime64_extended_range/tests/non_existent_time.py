...
@TestScenario
@Requirements(RQ_SRS_010_DateTime64_ExtendedRange_NonExistentTime_InvalidDate("1.0"))
...
@TestOutline(Suite)
@Requirements(RQ_SRS_010_DateTime64_ExtendedRange_NonExistentTime_InvalidTime("1.0"))
@Examples(
...
@Examples(
    ...
    [
        (
            "America/Denver",
            {"02:30:00": ("2018-03-11", "2020-03-08", "1980-04-27", "1942-02-09")},
        ),
        (
            "Europe/Zurich",
            {
                "02:30:00": ("2016-03-27", "2020-03-29", "1981-03-29"),
                "01:30:00": ("1942-05-04",),
            },
        ),
    ],
            ...
            with Step("Assignment test"):
                ...
                with Then(f"I check skipped hour"):
                    select_check_datetime(
                        datetime=dt_str, expected=expected, timezone=tz
                    )
...
@TestOutline(Scenario)
@Requirements(RQ_SRS_010_DateTime64_ExtendedRange_NonExistentTime_LeapSeconds("1.0"))
@Examples(
    "datet years",
    [
        (
    ...
    for timezone in timezones:
        ...
        with Step(f"{timezone}"):
            ...
            for i in range(len(transition_times) - 1, 0, -1):
                if (
                    (transition_times[i] > datetime.datetime.now())
                    or (transition_times[i].year < 1925)
                    or (transition_times[i].year > 2238)
                ):
                    continue
                with Step(f"{transition_times[i]}"):
                    ...
                    with And("compute expected result using Pytz"):
                        seconds_shift = (
                            transition_info[i][0] - transition_info[i - 1][0]
                        )
...
@Requirements(RQ_SRS_010_DateTime64_ExtendedRange_NonExistentTime("1.0"))

...
@TestScenario
def is_not_in_set_alias_in_watermark(self):
    """
    Define parameters for test case and call main outline.
    Test alias: is_not_in_set ALIAS value NOT IN (1, 2, 3, 4, 5)
    ...
    test_queries = [
        ...
        "SELECT is_not_in_set FROM {hybrid_table} ORDER BY id",
        "SELECT id, value, is_not_in_set FROM {hybrid_table} ORDER BY id",
        "SELECT id, value, is_not_in_set FROM {hybrid_table} WHERE is_not_in_set = 1 ORDER BY id",
    ]
    order_by = "(date_col, id)"
    partition_by = "toYYYYMM(date_col)"
...
def feature(self, minio_root_user=None, minio_root_password=None):
    ...
    Scenario(run=is_not_in_set_alias_in_watermark)

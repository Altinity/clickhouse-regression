...
@TestScenario
def multiple_alias_in_watermark(self):
    """
    Define parameters for test case and call main outline.
    Test alias: multiple ALIAS (value1 * value2 - value3) % (value2 + 1)
    ...
    test_queries = [
        ...
        "SELECT multiple FROM {hybrid_table} ORDER BY id",
        "SELECT id, value1, value2, value3, multiple FROM {hybrid_table} ORDER BY id",
        "SELECT id, value1, value2, value3, multiple FROM {hybrid_table} WHERE multiple >= 5 ORDER BY id",
    ]
    order_by = "(date_col, id)"
    partition_by = "toYYYYMM(date_col)"
...
def feature(self, minio_root_user=None, minio_root_password=None):
    ...
    Scenario(run=multiple_alias_in_watermark)

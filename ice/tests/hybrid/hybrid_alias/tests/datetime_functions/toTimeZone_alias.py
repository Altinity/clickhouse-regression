...
@TestScenario
def toTimeZone_alias_in_watermark(self):
    """
    Define parameters for test case and call main outline.
    Test alias: to_timezone ALIAS toTimeZone(datetime_col, 'America/New_York')
    ...
    watermark = {
        "left_predicate": "to_timezone >= '2010-01-01 00:00:00'",
        "right_predicate": "to_timezone < '2010-01-01 00:00:00'",
    }
    expected = {"exitcode": 0, "error_message": None}
    test_queries = [
        "SELECT id, value, date_col FROM {hybrid_table} ORDER BY id",
        "SELECT to_timezone FROM {hybrid_table} ORDER BY id",
        "SELECT id, value, to_timezone FROM {hybrid_table} ORDER BY id",
        "SELECT id, value, to_timezone FROM {hybrid_table} WHERE to_timezone >= '2010-01-01 00:00:00' ORDER BY id",
    ]
    order_by = "(date_col, id)"
    partition_by = "toYYYYMM(date_col)"
...
def feature(self, minio_root_user=None, minio_root_password=None):
    ...
    Scenario(run=toTimeZone_alias_in_watermark)

...
echo "Review new fails (brisk) at https://$artifact_s3_bucket_path.s3.amazonaws.com/$artifact_s3_dir/$(uname -i)/$SUITE$STORAGE/brisk-new-fails.log.txt"
echo "Review new fails (nice) at https://$artifact_s3_bucket_path.s3.amazonaws.com/$artifact_s3_dir/$(uname -i)/$SUITE$STORAGE/nice-new-fails.log.txt"

    ...
    for node_num in range(1, len(nodes) + 1):
        for shards_num in range(1, node_num + 1):
            ...
            for part_idx, partition in enumerate(all_partitions):
                ...
                cluster_name = f"cluster_{node_num}shards_{shards_num}replicas_{part_idx}"
                cluster_config = []
...
def add_clusters_for_nodes(self, nodes, modify=False):
    ...
    """
    return add_config(
        config=create_xml_config_content(root="yandex", entries=get_clusters_config_for_nodes(nodes=nodes), 
...
def get_clusters_for_nodes(self, nodes):
    ...
    return [i for i in get_clusters_config_for_nodes(nodes=nodes)["remote_servers"].keys()]
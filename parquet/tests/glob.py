...
def select_with_glob(self, query, snapshot_name):
    ...
    with When("I run the SELECT query with the glob pattern inside the Parquet file path"):
        node.query(query)

@TestScenario
@Examples("query snapshot_name", rows=[
...
@Examples("query snapshot_name", rows=[
    ...
    (f"SELECT * FROM file('{glob3}/?/dir/*{{p,a,r,q,u,e,t}}', Parquet)", "nested_glob_4"),

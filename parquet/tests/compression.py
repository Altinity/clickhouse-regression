...
@Requirements(
    RQ_SRS_032_ClickHouse_Parquet_Compression_Brotli("1.0")
)
def brotli(self):
    """Currently ClickHouse doesn't support reading Parquet files with brotli compression, but it will be added in
    the next release 23.3. (In regression we are using 22.8.15, maybe need to update?)"""
...
def gzip_pages(self):
    for page_number in range(1, 3):
        with Given(f"I have a parquet page={page_number} file with gzip page1 compression"):
            import_file = os.path.join("compression", f"data_page={page_number}_GZIP.parquet")

        import_export(snapshot_name=f"gzip_page_{page_number}_structure", import_file=import_file)
...
def lz4pages(self):
    for page_number in range(1, 3):
        with Given(f"I have a parquet page={page_number} file with lz4 compression"):
            import_file = os.path.join("compression", f"data_page={page_number}_LZ4.parquet")

        import_export(snapshot_name=f"lz4_page_{page_number}_structure", import_file=import_file)
@TestScenario
@Requirements(
...
def nonepages(self):
    for page_number in range(1, 3):
        with Given(f"I have a parquet page={page_number} file with None compression"):
            import_file = os.path.join("compression", f"data_page={page_number}_NONE.parquet")

        import_export(snapshot_name=f"none_page_{page_number}_structure", import_file=import_file)
...
def snappypages(self):
    """need to investigate if snappy should or should not be supported for both read and write"""
    for page_number in range(1, 3):
        with Given(f"I have a parquet page={page_number} file with Snappy compression"):
            import_file = os.path.join("compression", f"data_page={page_number}_SNAPPY.parquet")

        import_export(snapshot_name=f"snappy_page_{page_number}_structure", import_file=import_file)
...
def zstdpages(self):
    ...
    for page_number in range(1, 3):
        with Given(f"I have a parquet page={page_number} file with ZSTD compression"):
            import_file = os.path.join("compression", f"data_page={page_number}_ZSTD.parquet")

        import_export(snapshot_name=f"zstd_page_{page_number}_structure", import_file=import_file)

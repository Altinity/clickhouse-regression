...
@TestScenario
@Name("non_replicated_4_consumers")
def scenario(self):
    """Check that Kafka table engine can handle reading
    messages from Kafka in non-replicated mode
    under normal conditions with number of consumers set to 6
    and one materialized view attached that is writing to a MergeTree table.
    """

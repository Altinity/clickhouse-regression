...
(defn wait-clickhouse-alive!
  [node test clickhouse-alive? & {:keys [maxtries] :or {maxtries 90}}]
  (loop [i 0]
    (cond (> i maxtries)
          (throw (ex-info
                  (str "ClickHouse/Keeper on " node
                       " did not become alive after " maxtries
                       " tries (wrong binary arch, crash on start, or misconfig?)")
                  {:node node :maxtries maxtries}))

...
class S41(Scenario):
    ...
    def _measured_insert(self, ctx, result, *, node, table, policy, cols, rows, partitions,
        ...
        C.create_ca_table(node, table, columns=ddl_cols, order_by="c01",
    ...
    def run(self, ctx, result):
        ...
        smp = C.RssSampler(cl, interval_s=3.0)
        smp.start()
        try:
        ...
        C.record_peak_memory(result, smp, label="peak MemoryResident during wide inserts")
        ...
        try:
            C.drop_table_both(cl, "s41_plain")
        except Exception as e:
            ctx.log(f"S41: drop s41_plain failed (non-fatal): {e}")
        end = C.checkpoint_view(cl, result, ["s41_ca"])
        O.assert_fsck_clean(

...
class S36(Scenario):
    ...
    def run(self, ctx, result):
        ...
        for n in cl.nodes():
            C.create_ca_table(n, table, columns="id UInt64, pk UInt8, payload String",
        ...
        oracle_before = cl.node1.query(C.table_checksum_query(table)).strip()
        ...
        counters_to_ca = C.counters_window(cl)
        try:
        ...
        fsck_to_ca = C.run_cas_fsck(detail=False)
        ...
        oracle_after_to_ca = cl.node1.query(C.table_checksum_query(table)).strip()
        ...
        counters_off_ca = C.counters_window(cl)
        try:
        ...
        oracle_after_off_ca = cl.node1.query(C.table_checksum_query(table)).strip()
        ...
        fsck_off_ca = C.run_cas_fsck(detail=False)
        ...
        for t in (dedup_table_a, dedup_table_b):
            for n in cl.nodes():
                C.create_ca_table(n, t, columns="id UInt64, payload String", order_by="id",
                                    extra_settings={"storage_policy": "'ca_local'"})
        ...
        C.insert_values(cl.node1, dedup_table_a, dedup_gen, timeout=300)
        C.insert_values(cl.node1, dedup_table_b, dedup_gen, timeout=300)
        ...
        counters_dedup_a = C.counters_window(cl)
        ...
        counters_dedup_b = C.counters_window(cl)
        ...
        oracle_dedup_a = cl.node1.query(C.table_checksum_query(dedup_table_a)).strip()
        oracle_dedup_b = cl.node1.query(C.table_checksum_query(dedup_table_b)).strip()
        result.add(Verdict.check(
        ...
        C.insert_values(cl.node1, table, gen_chaos, timeout=600)
        ...
        if not chaos_placement_before:
            ...
            oracle_before_chaos = cl.node1.query(C.table_checksum_query(table)).strip()
            move_error = {}
            ...
            healthy = O.wait_cluster_healthy(cl, timeout_s=heal_timeout_s, log_fn=ctx.log)
            ...
            if not healthy:
                ...
                oracle_after_chaos = cl.node1.query(C.table_checksum_query(table)).strip()
        ...
        end = C.checkpoint_view(cl, result, [table])
        O.assert_fsck_clean(
...
class S37(Scenario):
    ...
    def run(self, ctx, result):
        ...
        for n in cl.nodes():
            C.create_ca_table(n, route_table, columns="id UInt64, payload String", order_by="id",
                                extra_settings={"storage_policy": "'ca_local'"})
        C.insert_random(cl.node1, route_table, rows=small_rows, payload_bytes=small_payload, op_id=0)
        C.insert_random(cl.node1, route_table, rows=big_rows, payload_bytes=big_payload,
                          op_id=10_000_000)
        ...
        for n in cl.nodes():
            C.create_ca_table(n, ttl_table, columns="id UInt64, ts DateTime, payload String",
        ...
        for n in cl.nodes():
            C.create_ca_table(n, mixed_table, columns="id UInt64, payload String", order_by="id",
                                extra_settings={"storage_policy": "'ca_local3'"})
        for i in range(mixed_parts):
            C.insert_random(cl.node1, mixed_table, rows=mixed_rows, payload_bytes=mixed_payload,
                              op_id=i * mixed_rows)
        ...
        counters_merge = C.counters_window(cl)
        ...
        oracle_mixed = cl.node1.query(C.table_checksum_query(mixed_table)).strip()
        ...
        healthy = O.wait_cluster_healthy(cl, timeout_s=restart_timeout_s, log_fn=ctx.log)
        ...
        if not healthy:
            ...
            oracle_mixed_after = cl.node1.query(C.table_checksum_query(mixed_table)).strip()
        ...
        gen_chaos = (f"SELECT number AS id, now() - 10 AS ts, randomString({ttl_payload}) AS payload "
                    f"FROM numbers({ttl_rows})")
        C.insert_values(cl.node1, ttl_table, gen_chaos, timeout=600)
        ...
        oracle_before_chaos = cl.node1.query(C.table_checksum_query(ttl_table)).strip()
        move_error = {}
        ...
        mover_thread.join(timeout=60)
        healthy_chaos = O.wait_cluster_healthy(cl, timeout_s=restart_timeout_s, log_fn=ctx.log)
        result.observations["chaos_move_error"] = move_error.get("err")
        ...
        if not healthy_chaos:
            ...
            oracle_after_chaos = cl.node1.query(C.table_checksum_query(ttl_table)).strip()
        ...
        end = C.checkpoint_view(cl, result, all_tables, table_filter="table LIKE 's37_%'")
        O.assert_fsck_clean(
            result, end.get("fsck_final"),

"""S23 idle pool, S24 small dedup cache, S25 Ordinary paths, S26 file churn, S27 LIST proxy (P2).

S24 and S27 still need their compose variants at runtime. The bodies are the tests.
"""
...
def _list_ca_tables(node):
    try:
        ...
        return []
    return [line for line in (txt or "").splitlines() if line]
    ...
    def run(self, ctx, result):
        ...
        result.add(
            Verdict(
                ...
                f"(scale={ctx.scale})",
                "pass",
                "dev/ci shorten the idle window; only --scenario-scale full approaches 15 minutes",
            )
        )
        result.add(
            Verdict.inconclusive(
                "1-server idle baseline",
                "1-server config measured",
                "compose fixed at 2 servers",
            )
        )
        result.add(
            Verdict.inconclusive(
                "10-server idle baseline",
                "10-server config measured",
                "compose fixed at 2 servers",
            )
        )

        leftover = _list_ca_tables(cl.node1)
        ...
        per_minute = []
        sampler = C.RssSampler(cl, interval_s=max(1.0, minute_s / 3.0))
        sampler.start()
        try:
            for minute in range(idle_minutes):
                ...
                if mem_before is None:
                    mem_before = C.cluster_memory(cl)
                    result.observations["server_memory_before"] = mem_before
                    base_rss = max(
                        (m.get("mem_resident") or 0 for m in mem_before.values()),
                        default=0,
                    )
        finally:
            sampler.stop()
        result.observations["per_minute_idle_gc"] = per_minute
        C.record_peak_memory(result, sampler, label="peak MemoryResident while idle")

        delta = counters().get("_total", {})
        result.observations["idle_gc_op_counters"] = {
            k: int(delta.get(k, 0)) for k in _IDLE_COUNTERS
        }
        ...
        if per_round_ops:
            result.add(
                Verdict.check(
                    ...
                    ""
                    if max_round_ops <= budget
                    else "an idle empty-pool GC round issued more object-store ops than the budget",
                )
            )
        else:
            result.add(
                Verdict.inconclusive(
                    "idle GC S3 ops per round below budget",
                    f"<= {budget} per round",
                    "no idle GC rounds were measured",
                )
            )
        ...
        result.add(
            Verdict.check(
                "no Failed idle GC rounds",
                "GC log has 0 Error finish rows on an idle pool",
                failed,
                failed == 0,
                "" if failed == 0 else "idle GC produced Error finish rows",
            )
        )
        result.add(
            Verdict(
                "NotALeader rounds expected & quiet",
        ...
        ca_events = O.ca_event_counts_all(cl, ctx.extra.get("since_event_time"))
        bad_total = ca_events.get("bad_total", {})
        result.observations["idle_ca_bad_events"] = bad_total
        result.add(
            Verdict.check(
                "no noisy CA exceptions while idle",
                "no read_missing/exception/... CA-log rows",
                bad_total,
                not bad_total,
        ...
        if base_rss > 0:
            result.observations["idle_rss_growth"] = after_rss - base_rss
        if tracked_growth:
            ...
            result.add(
                Verdict.check(
                    ...
                    ""
                    if ok
                    else "tracked server memory grew over an idle window with no workload",
                )
            )
        else:
            result.add(
                Verdict.inconclusive(
                    "memory flat over idle window",
                    "per-node MemoryTracking growth bounded",
                    "could not read MemoryTracking before and after the idle window",
                )
            )

        end = C.standard_end(cluster=cl, result=result, tables=[])
        O.assert_fsck_clean(
            result,
            end,
            name="idle pool fsck clean",
            expected="fsck dangling==0 on the empty pool",
            fail_note="an idle empty pool reported dangling refs",
        )
...
class S24(Scenario):
    ...
    def run(self, ctx, result):
        ...
        for n in cl.nodes():
            C.create_ca_table(n, table, columns="id UInt64, payload String",
        ...
        fill_counters = C.counters_window(cl)
        ...
        hot_counters = C.counters_window(cl)
        ...
        C.checkpoint_view(cl, result, [table])
...
class S25(Scenario):
    ...
    def run(self, ctx, result):
        ...
        try:
            self._run(ctx, result, cl, rows, payload, dbname, table)
        finally:
            for n in cl.nodes():
                try:
                    n.command(f"DROP DATABASE IF EXISTS {dbname} SYNC", timeout=120)
                except Exception as e:
                    ctx.log(f"S25: drop database on {n.container}: {e}")

    def _run(self, ctx, result, cl, rows, payload, dbname, table):
        ordinary_ok = False
        create_error = None
        for n in cl.nodes():
            try:
                ...
                ctx.log(f"S25: pre-drop of {dbname} on {n.container}: {str(e)[:160]}")
        try:
            for n in cl.nodes():
                n.command(
        ...
        if not ordinary_ok:
            result.add(
                Verdict.inconclusive(
                    ...
                    "Ordinary database engine is deprecated or blocked in this build: "
                    f"{create_error}",
                )
            )
            result.note_anomaly(
                "S25 could not create an Ordinary database; the non-Atomic path was not exercised"
            )
        ...
        C.create_ca_table(
            ...
            order_by="id",
            wide=True,
            replica_path=f"/clickhouse/tables/{dbname}_s25_ordinary",
        )
        C.insert_random(cl.node1, table, rows=rows, payload_bytes=payload, op_id=0)
        C.insert_random(cl.node1, table, rows=rows, payload_bytes=payload, op_id=rows)

        try:
            part = cl.node1.scalar(
                "SELECT name FROM system.parts WHERE database='s25db' "
                "AND table='s25_ordinary' AND active ORDER BY name LIMIT 1"
            )
            if part:
            ...
            ctx.log(f"S25: detach/attach: {str(e)[:200]}")
        ...
        try:
            ...
            ctx.log(f"S25: freeze: {str(e)[:200]}")
        ...
        try:
            ...
            ctx.log(f"S25: mutation: {str(e)[:200]}")
            result.observations["s25_mutation_error"] = str(e)[:1000]

        pool = C.pool_shape(timeout_s=120)
        ...
        try:
            ...
            ctx.log(f"S25: drop partition: {str(e)[:200]}")
        try:
            cl.node1.command("SYSTEM UNFREEZE WITH NAME 's25_freeze'", timeout=300)
        except Exception as e:
            ctx.log(f"S25: unfreeze: {str(e)[:200]}")
        ...
        O.assert_fsck_clean(
            ...
            fail_note="dangling refs survived the non-Atomic-db lifecycle",
        )
...
class S26(Scenario):
    ...
    def run(self, ctx, result):
        ...
        C.create_ca_table(
            cl.node1,
            table,
            columns="id UInt64, payload String, tag UInt32",
            order_by="id",
            wide=True,
        )
        C.insert_random(
            cl.node1,
            table,
            rows=rows_per_insert * 4,
            payload_bytes=payload,
            op_id=0,
            extra_cols_select="toUInt32(number % 7) AS tag",
        )
        pool_before = C.pool_shape(timeout_s=120)
        ...
        for i in range(alters):
            try:
                cl.node1.command(
                    f"ALTER TABLE {table} MODIFY COLUMN tag COMMENT 'c{i}'", timeout=120
                )
            except Exception as e:
                alter_failures += 1
                if alter_failures <= 5:
                    ctx.log(f"S26: ALTER {i}: {str(e)[:160]}")
        result.observations["s26_alter_failures"] = alter_failures

        ident = (
            f"SELECT number AS id, repeat('x', {payload}) AS payload, "
            f"toUInt32(number % 7) AS tag FROM numbers({rows_per_insert})"
        )
        for _ in range(dedup_inserts):
            C.insert_values(cl.node1, table, ident, timeout=300)
        delta = counters().get("_total", {})
        cas_blob = {
            k: int(delta.get(k, 0))
            for k in (
        ...
        pool_after = C.pool_shape(timeout_s=120)
        ...
        if files_before is not None and files_after is not None:
            result.add(
                Verdict(
                    "verbatim _files churn observed",
                    "table-level _files objects present during churn",
                    f"_files {files_before} -> {files_after}",
                    "pass",
                    "table-level files live under _files, not as content blobs",
                )
            )
        body_puts = cas_blob["CASBlobPut"]
        avoided = (
            cas_blob["CASBlobBodyPutAvoided"]
            + cas_blob["CASBlobPutDeduplicated"]
        ...
        result.add(
            Verdict.check(
                ...
                ""
                if (avoided > 0 or body_puts <= 4)
                else "identical inserts kept uploading new blob bodies",
            )
        )
        for n in cl.nodes():
            try:
                ...
                ctx.log(f"S26: SYNC REPLICA on {n.container}: {e}")
        C.assert_replicas_agree(
            result, cl, C.table_checksum_query(table), name="S26 replica agreement"
        )

        before_drop = C.counters_window(cl)
        C.drop_table_both(cl, table)
        result.observations["tables"] = []
        C.gc_drive_round(cl, log_fn=ctx.log)
        drop_delta = before_drop().get("_total", {})
        pool_dropped = C.pool_shape(timeout_s=120)
        ...
        if files_after is not None and files_dropped is not None:
            ...
            result.add(
                Verdict.check(
                    ...
                    "the dropped table's _files objects drain",
                    f"_files {files_after} -> {files_dropped} after drop",
                    drained,
                    ""
                    if drained
                    else "dropping the table did not drain its table-level _files objects",
                )
            )
        result.add(
            Verdict(
                "regular GC not implicated for _files",
                "_files removed by owner path, not via CASBlobDelete",
                f"CASBlobDelete during drop+GC = {int(drop_delta.get('CASBlobDelete', 0))}",
                "pass",
                "CASBlobDelete on drop reclaims data blobs; table-level _files are removed with the namespace",
            )
        )
        end = C.standard_end(cluster=cl, result=result, tables=[])
        O.assert_fsck_clean(
            ...
            fail_note="dangling refs after verbatim-file churn",
        )
...
class S27(Scenario):
    ...
    def run(self, ctx, result):
        ...
        for t in tables:
            ...
            C.insert_random(nodes[0], t, rows=rows, payload_bytes=payload, op_id=0)

        # Baseline GC round with STABLE listing → reference discovery cost.
        base_before = C.counters_window(cl)
        C.gc_drive_round(cl, log_fn=ctx.log)
        ...
        anomaly_before = C.counters_window(cl)
        gc_errors = []
        ...
        C.checkpoint_view(cl, result, [t for i, t in enumerate(tables) if i % 2 == 1],

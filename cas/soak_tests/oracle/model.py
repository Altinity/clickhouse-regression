from .ledger import OpType
from .rng import MASK64
from .rowgen import BASE_TIME, NBUCKETS, insert_rids, row_for_rid
...
class Model:
    ...
    def __init__(
        ...
        ttl_seconds: int = 90 * 60,
        insert_block: int = 200,
    ):
        self.seed = seed
    ...
    def apply(self, op):
        if op.type == OpType.INSERT:
            ...
            self.rows.clear()  # single BASE_TIME day -> one partition -> drop == full clear
    ...
    def ambiguous_band_nonempty(self, now: int, eps: int) -> bool:
        return any(
            abs((r["ts"] + self.ttl_seconds) - now) <= eps for r in self.rows.values()
        )

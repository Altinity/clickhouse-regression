...
- [ ] **CAS-001** Reader holds no pin across the deferred, unpinned blob GET → a `dropRef`+GC condemn→delete can remove a committed blob mid-query for ref-… · `DATA-LOSS` — resolution: 🔴 still-present — ``Parts/PartFolderAccess.cpp:270,295`; `Gc/CasGc.cpp:367``
- [ ] **CAS-002** Shard `casPut` fenced by content token, not `writer_epoch` → pause/TOCTOU/clone/clock-skew opens a dual-writer window (split-brain / zomb… · `SECURITY / CORRECTNESS` — resolution: 🔴 still-present — ``Pool/CasMountRuntime.cpp:81-142`; `Backend/CasRequestControl.cpp:225-611`; `Gc/CasGc.cpp:3002,3026,3043``
- [ ] **CAS-003** Non-cryptographic content hash (CityHash128) + reads never re-verify → collision-based blob poisoning across a shared pool via pool-globa… · `SECURITY / INTEGRITY` — resolution: 🔴 still-present — ``Pool/CasPartWriteTxn.cpp:77`; `ContentAddressedTransaction.h:291`; opt-in sha256 at `ContentAddressedSettings.cpp:74``
- [ ] **CAS-004** No intra-pool authorization; identities self-asserted → bucket credential is the whole perimeter; a pool-write peer can forge mounts, tam… · `SECURITY` — resolution: 🔴 still-present — ``Pool/CasServerRoot.h:305-329`; single-credential bucket assumption pool-wide`
- [ ] **CAS-005** Blob payload never re-hashed against `logical_hash` on the normal read path → silent S3 bit-rot / truncation undetected by CAS; integrity… · `INTEGRITY` — resolution: 🔴 still-present — ``ContentAddressedMetadataStorage.cpp:1923-1933`; `Pool/CasPartWriteTxn.cpp:77``
- [ ] **CAS-006** CAS durable publish runs under `DataPartsLock` (blob PUTs + precommit + promote + ref CAS, each with retries) → table-wide writer/DDL sta… · `LIVENESS / PERF` — resolution: 🔴 still-present — ``ContentAddressedTransaction.cpp:435, 485-500`; `:480-484` "serially"`
- [ ] **CAS-007** UniqueKey / upsert MergeTree: DeleteBitmap + SSTIndex hot-rewrite — delete-bitmap is not in the mutable-per-part set, so every `replaceFi… · `FEATURE-GAP / PERF` — resolution: 🔴 still-present — ``ContentAddressedTransaction.cpp:65-73, :1491-1507``
- [ ] **CAS-008** Untrimmed root-shard journal under churn hits the 64 MiB hard limit → writes rejected until GC folds/trims. Couples write availability to… · `LIVENESS / SCALE` — resolution: 🔴 still-present — ``Formats/CasRefLogFormat.h:68-79`, `Pool/CasRefLedger.cpp:572-577`, `Pool/CasPartWriteTxn.cpp:824-872``
- [ ] **CAS-009** Rolling upgrade across a format-generation bump breaks old nodes — `compatibility_version` always stamped at `G_BUILD` (write-down-to-flo… · `COMPAT` — resolution: 🔴 still-present — ``Formats/CasFormat.cpp:57-62`; `CasFormat.h:28` (`G_BUILD=3`)`
- [ ] **CAS-010** No coverage-guided fuzzing of any CAS decoder despite untrusted shared-pool bytes (envelope, run-file, manifest, root-shard, gc-formats, … · `TEST-GAP / DoS` — resolution: 🔴 still-present — ``src/Disks/fuzzers/` absent; no `LLVMFuzzerTestOneInput` for envelope / run-file / manifest / root-shard / gc-formats / pool-meta in `src/`. Only unit tests: `s`
- [ ] **CAS-011** Bucket versioning / GCS soft-delete silently breaks GC reclaim; guarded only when the versioning API is queryable, else fails open (mount… · `OBSERV / LEAK` — resolution: 🔴 still-present — ``Backend/CasObjectStorageBackend.cpp:55-84` (Native+Generation-only; fail-open on inconclusive; S3 versioning not covered)`
- [ ] **CAS-012** Native conditional-write path not end-to-end tested on real S3/GCS (only RustFS + emulation) — the single most safety-critical untested s… · `TEST-GAP` — resolution: 🔴 still-present — `Only mock: `src/Disks/tests/gtest_cas_backend_generation.cpp`, `gtest_cas_backend_contract.cpp`. No `conditional`/`IfNoneMatch` string in `tests/integration/tes`
- [ ] **CAS-013** `fsck` (reachability / dangling / physical-vs-logical) is not operator-accessible via SQL — the most valuable health diagnostic is intern… · `OBSERV/DAY2` — resolution: 🛠 will-fix — ``Parsers/ASTSystemQuery.h:153`, `InterpreterSystemQuery.cpp:2524-2551``
- [ ] **CAS-014** No GC-liveness / reclaim-backlog / physical-bytes metric — the most common silent failure ("GC stopped reclaiming") is invisible without … · `OBSERV/DAY2` — resolution: 🛠 will-fix — ``StorageSystemContentAddressedMounts.cpp:52-55`, `Gc/CasGcScheduler.h:118-125``
- [ ] **CAS-015** `GC REBUILD` has no mount-lease interlock → run against a live pool, its non-atomic universe/blob scans can bless a baseline missing a co… · `DATA-LOSS` — resolution: 🔴 still-present — ``CasGc.cpp:2488–2568`; `ContentAddressedMetadataStorage.cpp:621–655`; `CommandCaGcRebuild.cpp:54``
- [ ] **CAS-016** Lifecycle expiration rule deletes live blobs (age-based rules hit the oldest, most-shared blobs) → dangling refs / data loss. Unguarded. · `DATA-LOSS / CONFIG` — resolution: 🔴 still-present — ``Backend/CasObjectStorageBackend.cpp:55-84` (no lifecycle-rule inspection anywhere in CAS tree)`
- [ ] **CAS-017** Object Lock / WORM / retention breaks CAS entirely — mutable root-shards and `gc/state` cannot be overwritten → all writes + GC fail. Ung… · `CONFIG / LIVENESS` — resolution: 🔴 still-present — ``Backend/CasObjectStorageBackend.cpp:55-84` + mutable control plane at `Pool/CasServerRoot.cpp:575-645`, `Gc/CasGc.cpp:673`, `Pool/CasPartWriteTxn.cpp:508-612``
- [ ] **CAS-018** No bounded delete→physical-erase SLA — reclaim is GC-deferred and stallable indefinitely; cannot promise GDPR/CCPA erasure deadlines from… · `COMPLIANCE` — resolution: 🔴 still-present — ``ContentAddressedTransaction.cpp:1034`; no SLA setting in `ContentAddressedSettings.cpp``
- [ ] **CAS-019** Dedup means one owner's delete may erase nothing while any other ref shares the byte-identical blob; no per-subject "shred everywhere" pr… · `COMPLIANCE` — resolution: 🔴 still-present — ``Gc/CasBlobInDegree.cpp:364-448`; `Pool/CasRefLedger.cpp:2801,2890``
- [ ] **CAS-046** `DiskEncrypted` random-IV ciphertext defeats content-addressed dedup — every encrypted file becomes a unique blob; CAS's whole value null… · `FEATURE-GAP` — resolution: 🔴 still-present — `absence in `Backend/CasObjectStorageBackend.cpp`; `DiskEncrypted` random IV`

## 🟠 Medium

- [x] **CAS-020** `promote`-overwrite leaks the prior committed manifest (unconditional `refs[R]=…`); reachable via RENAME / lost-ACK replay. The only non-… · `LEAK` — resolution: ✅ fixed — ``Pool/CasRefProtocol.cpp:262-265`, `Parts/PartFolderAccess.cpp:520-529``
...
- [x] **CAS-026** Protobuf `ParseFromArray` with unchecked `static_cast ` and no size cap → OOM / negative-size on a corrupt/planted oversized object; no d… · `DECODE/DoS` — resolution: ✅ fixed — ``Formats/CasTextFormat.cpp:389,401` (`object_cap` pre-decode); no `ParseFromArray` in tree`
...
- [x] **CAS-028** `RunFileReader::next()` parses record `klen`/`plen` with unchecked `operator[]`/`substr` → OOB heap read / non-`CORRUPTED_DATA` throw on … · `DECODE/DoS` — resolution: ✅ fixed — ``Formats/CasRecordStreamFormat.cpp:228–304` (NDJSON `readLine` w/ `line_cap`); no `klen`/`plen``
- [ ] **CAS-029** VM-clone / snapshot split brain — two live servers share one `server_uuid`; dual mount bounded only by renew period. · `CORRECTNESS` — resolution: 🔴 still-present — ``Pool/CasServerRoot.cpp:806-830`, `Pool/CasMountRuntime.cpp:133``
- [ ] **CAS-030** Wall-clock mount-lease expiry vs boot-clock local fence → clock skew → premature reclaim / false unavailability (token-guarded, no corrup… · `CORRECTNESS / SECURITY` — resolution: 🔴 still-present — ``Pool/CasServerRoot.cpp:217,276``
...
- [ ] **CAS-033** Persistent shard clamp → pool-wide `suppress_destructive` halts all reclamation with no self-heal (safety-preserving). · `LIVENESS` — resolution: 🔴 still-present — ``Gc/CasGc.cpp:1833-1842`; `Gc/CasGc.cpp:2079-2080`; `Gc/CasGc.cpp:2170-2171``
...
- [ ] **CAS-035** Presence-asserting closures misreport a lost-ACK-succeeded write as failure (e.g. `dropRef` re-reads its own committed drop → `FILE_DOESN… · `CORRECTNESS` — resolution: 🔴 still-present — ``Pool/CasRefLedger.cpp:2811-2814``
...
- [x] **CAS-037** Content-hash algorithm is an unversioned, unpinned pool contract (not recorded in `PoolMeta`); a future CityHash/algorithm change silentl… · `INTEGRITY / COMPAT` — resolution: ✅ fixed — ``Formats/CasPoolMetaFormat.h:29-31`; `Pool/CasPoolMeta.cpp:56-102, 148`; `Formats/CasLayout.cpp:36``
...
- [ ] **CAS-045** ZK part-set vs CAS ref can diverge on partial commit (ZK-has/CAS-missing → broken part; CAS-has/ZK-missing → invisible live-ref leak). · `CORRECTNESS / LEAK` — resolution: 🔴 still-present — `out-of-CAS (RMT integration)`
...
- [ ] **CAS-082** Lost-ACK replay double-appends journal events (set-idempotent → journal bloat only). · `LEAK` — resolution: 🔴 still-present — ``Pool/CasRefLedger.cpp:1842–1861` (idempotent no-op tolerated but retried ops still appended to journal)`
- [ ] **CAS-083** Flat-combining leader convoy + batch-wide failure amplification under S3 stall (latency, not correctness). · `LIVENESS` — resolution: 🔴 still-present — ``Pool/CasRefLedger.cpp:1131-1206` leader convoy + batch-wide failure`
- [ ] **CAS-084** Orphan multipart uploads / ownerless manifest bodies on interrupt — reclaimed by S3 lifecycle + watermark sweep; CAS neither aborts nor r… · `LEAK / DAY2` — resolution: 🔴 still-present — `absence: no MPU calls under CAS `Tools/`,`Gc/`,`Pool/` (only `Backend/CasObjectStorageBackend.cpp:813-828`)`
- [x] **CAS-085** `allow_stale` decode-TTL ↔ GC condemn→delete latency coupling is a convention, not an enforced invariant. · `CORRECTNESS` — resolution: ✅ fixed — ``Pool/CasRefLedger.cpp:174-179``
- [ ] **CAS-086** `readManifest` HEAD+GET is not coalesced and absence not negatively cached → HEAD+GET storm under throttling. · `PERF` — resolution: 🔴 still-present — ``Pool/CasManifestReader.cpp:56-137``
...
- [x] **CAS-088** Lost/corrupt GC-internal artifacts wedge GC until manual `GC REBUILD` (by-design fail-stop-then-recover). · `LIVENESS` — resolution: 📐 by-design — ``Gc/CasGc.cpp:2488-2903` (rebuild is the recovery); `CasGcScheduler.cpp:254-278``
- [ ] **CAS-089** Regular-round mass-drop delta is a non-streaming in-memory point (rebuild is batched; regular round isn't). · `PERF/SCALE` — resolution: 🔴 still-present — ``Gc/CasGc.cpp:1362, 1582, 1880-1882` (regular round intake not streamed)`
...
- [ ] **CAS-110** FETCH-to-detached never relinks (full byte transfer even same-pool); quorum/SYNC REPLICA/cloneReplica correct-by-composition but untested. · `PERF / TEST-GAP` — resolution: 🟡 partial/mitigated — `SYNC REPLICA covered (`tests/integration/test_cas_replicated_relink/test.py` lines 284, 309, 313, 318, 355, 519, 551, 557, 616, 684, 798, 868). Quorum + cloneRe`
...
- [x] **CAS-211** Repudiation: provenance/`CasEvent` are self-asserted (forgeable by a pool-write adversary); blobs plaintext / content-equality observable… · `SEC-8, SEC-9` — resolution: ⚪ info — ``Formats/CasBlobEnvelopeFormat.h:35-44``
...
- [ ] **NEW-AD1-3 (Info)** (?) CAS-025 fix incidentally lands here
...
- [ ] **NEW-ad3-2 (Low)** (?) `SYSTEM CONTENT ADDRESSED GC STOP` truthfully-but-misleadingly reports `is_leader`. Comment at `InterpreterSystemQuery.cpp:1018-1023` exp…
...
- [ ] **NEW-ad5-2 (Low)** (?) `enforceRefTableCacheBudget` LRU-evicts a namespace's cached state; a hot table churning enough to force re-hydration under memory pressu…
- [ ] **NEW-ad5-3 (Low)** (?) `RefLog` / `RefSnapshot` seal-decode ceilings are 64 MiB decompressed and enforced at decode, but the encode-side complete-table admissio…
- [ ] **NEW-AD6-1 (Low)** (?) GCS-versioning precondition treats "cannot verify" as fail-open by design, but the log level is `LOG_WARNING` inside `checkPoolPreconditi…
- [ ] **NEW-AD6-2 (Info)** (?) `Pool/CasPool.cpp:1327` bakes the "S3 strongly consistent since 2021" assumption in a comment, without a runtime capability check or a do…
- [ ] **NEW-ad7-1 (Info)** (?) `assertEOF` after `readStringBinary(sender_manifest_bytes)` will hard-fail any future v2-with-trailer sender talking to this exact v2 rec…
- [ ] **NEW-ad7-2 (Info)** (?) Cookie-value gate happens BEFORE the pool-uuid re-check, but AFTER `ca_relink` cookie parse — an empty cookie value on the wire is silent…
...
- [ ] **NEW-bc1-1 (Low)** (?) `decodeEnvelopeHeader` discards `object_size` but the payload extent still depends on `object_size >= h.header_len`, which is **not check…
...
- [ ] **NEW-BC2-7 (Low)** (?) `SCOPE_EXIT`-only cleanup covers throw but not a survivor if `stageBlobPartFile` succeeds and a later transaction step throws (inline-ove…
...
- [ ] **NEW-bc4-protobuf-decode-1 (Info)** (?) `Backend/CasBackend.h:219` has a stale doc comment referring to the deleted `RunFileReader`. Harmless; rename to "record-stream reader" w…
- [ ] **NEW-bc4-protobuf-decode-2 (Info)** (?) `ShardCoverage::classification` is decoded as an unvalidated `uint8_t` (`Formats/CasFoldSealFormat.cpp:189`). No consumer branches on it …
- [ ] **NEW-BC7-1 (Med)** (?) Asymmetric fix: replicated write paths use the `renameParts()` off-lock publish, but plain-MergeTree paths (`MergeTreeSink.cpp:379`, `Mut…
- [ ] **NEW-BC7-2 (Low)** (?) Belt-and-suspenders duplication: after `renameParts()` (`MergeTreeData.cpp:8986-8988`) runs the publish loop, the identical `commitTransa…
- [ ] **NEW-codeonly-line-1 (Low)** (?) Non-cryptographic checksum still trusted on run objects, now under `CityHash128`-of-object-body — `Formats/CasRecordStreamFormat.cpp:210-…
- [ ] **NEW-codeonly-line-2 (Info)** (?) Envelope pad-zone now enforces "must be ASCII space up to '\n'" — `CasBlobEnvelopeFormat.cpp:230-248`. Reads any non-space byte as `CORRU…
- [ ] **NEW-codeonly-line-3 (Low)** (?) `PartWriteTxn::adoptEvidence` records the sender-supplied `entry.blob_size` in `deps[entry.ref]` and then verifies only that a blob exist…
- [ ] **NEW-codeonly-line-4 (Info)** (?) `checkNamespace` is now also called on operator-supplied `server_root_id`-derived subpaths via a wide surface (`refsNamespacePrefix`, `ma…
- [ ] **NEW-datatype-agnosticism-1 (Info)** (?) original audit's Layer 1 quote used `UInt128 blob_hash{}`. In current code the hash + algorithm are packed into a single `BlobRef ref` (`…
- [ ] **NEW-datatype-agnosticism-2 (Info)** (?) original audit's edge-case table noted mutable per-part files (`uuid.txt`/`txn_version.txt`/`metadata_version.txt`) were kept out of the …
- [ ] **NEW-datatype-agnosticism-3 (Info)** (?) the only place CAS inspects a file's name to alter behavior is `Cas::partFileMustStayBlob` (`ContentAddressedTransaction.cpp:65-73`) whic…
...
- [ ] **NEW-jepsen-2 (Low — soft-vs-hard fence)** (?) `fenceGeneration` admission covers the durable-effect blob finalize path only for `ContentAddressedTransaction::writeFile`; the ref-appen…
...
- [ ] **NEW-read-1 (Med, liveness)** (?) `readManifestShared` HEAD-GET race window amplifies dangle. Anchor `Pool/CasManifestReader.cpp:65-90`. HEAD at l.65 and GET at l.87 are s…
- [ ] **NEW-read-2 (Low, coverage)** (?) Retained-view age policy uses wall-clock `now_ms_fn()` (`PartFolderAccess.cpp:203`) subtracted from `cached->validatedAtMs()`. On backwar…
- [ ] **NEW-read-3 (Info)** (?) `CachedPartFolderAccess::buildView` single-flight drops leader exceptions onto every follower (`l.301: promise.set_exception`). One HEAD …
- [ ] **NEW-security-1 (Med — default-hardening)** (?) `BlobHashAlgo` default is `CityHash128`
- [ ] **NEW-security-2 (Low — clean-shutdown)** (?) `Xxh3128BlobHashingWriteBuffer::finalizeImpl` is not overridden; `getHashHex` calls `next()` but skips `finalize()`, and the class also l…
...
- [ ] **NEW-security-4 (Low)** (?) `mountpointObjectKey` and `checkNamespace` accept single-character segments `.` and `..` (sub-finding of CAS-074, called out separately f…
- [ ] **NEW-TCF-1 (Med)** (?) no GCS-backed integration test despite `utils/ca-soak/docker-compose-gcs.yml` being present. All `tests/integration/test_cas*` and `test_…
- [ ] **NEW-TCF-2 (Low)** (?) `ca-soak/scenarios/` is model/checker-oriented but has no explicit **MOVE / RESTORE / quorum** cards even though it has multi-replica inf…
- [ ] **NEW-TCF-3 (Info)** (?) `gtest_cas_ref_decode_bounds.cpp` exists and is a natural seed corpus for a `cas_ref_decode_fuzzer`. Converting its hand-crafted malforme…
...
- [ ] **NEW-tier3-3 (Info)** (?) Same-pool same-disk `moveDirectory` for part-dirs is a pure metadata `republishRef` (`ContentAddressedTransaction.cpp:1370`), i.e. genuin…
- [ ] **NEW-tier4-1 (Low, test-coverage)** (?) FETCH-to-detached relink correctness has code but no dedicated integration test
- [ ] **NEW-tier4-2 (Low, test-coverage)** (?) RENAME TABLE / cross-engine table move on CAS is best-effort non-atomic and the "SPLIT across namespaces" recovery path is idempotent by …
- [ ] **NEW-tier4-3 (Low, observability)** (?) a `CasBlobAdoptTrusted` ProfileEvent (`src/Common/ProfileEvents.cpp:900`) partially closes the original OBS-3 gap (relink hit-rate is mea…
...
- [ ] **NEW-upgrade-compat-5 (Info)** (?) no LE-only build assertion. No occurrence of `std::endian::native` / `__BYTE_ORDER__` / `static_assert` guarding LE-only in `MetadataStor…
- [ ] **NEW-write-1 (Low, LEAK/OBSERV)** (?) `CaContentWriteBuffer::finalizeImpl` sets `temp_ownership_transferred = true` (line 1845) even after `on_finalized` throws in the tail of…
- [ ] **NEW-write-2 (Low, CORRECTNESS/COMPAT)** (?) `CaContentWriteBuffer::finalizeImpl` reports the payload size via `count()` (line 1822), which is the byte count of what THIS buffer forw…
- [ ] **NEW-write-3 (Low, LIVENESS/PERF)** (?) `ContentAddressedTransaction::moveDirectory`'s RENAME-TABLE branch (`:1231–1249`) does `for (const auto & [ref, _] : store->listRefs(from…

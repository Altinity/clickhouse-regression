...
def set_tiered_policy(self, allow_vfs=1, move_on_insert=0):
    ...
    s3_storage(
        disks=disks, policies=policies, restart=True, config_file="policy_settings.xml"
    )
...
@Tags("sanity")
@Requirements(
    RQ_SRS038_DiskObjectStorageVFS_Table_TTLDelete("1.0"),
    RQ_SRS038_DiskObjectStorageVFS_SharedSettings_TTL("1.0"),
)
...
@Tags("sanity")
@Requirements(
    RQ_SRS038_DiskObjectStorageVFS_Table_TTLMove("1.0"),
    RQ_SRS038_DiskObjectStorageVFS_SharedSettings_TTL("1.0"),
)
@Examples("move_on_insert", [[0], [1]])

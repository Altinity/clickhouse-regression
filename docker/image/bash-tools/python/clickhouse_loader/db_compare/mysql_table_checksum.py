...
def get_table_checksum_query(table, conn):
    ...
    (rowset, rowcount) = execute_mysql(
        conn,
        "select COLUMN_NAME as column_name, column_type as data_type, IS_NULLABLE as is_nullable from information_schema.columns where table_schema='"
        + args.mysql_database
        + "' and table_name = '"
        + table
    ...
    for row in rowset:
        ...
        if (
            "datetime" == data_type
            or "datetime(1)" == data_type
            or "datetime(2)" == data_type
            or "datetime(3)" == data_type
        ):
            ...
            "datetime(4)" == data_type
            or "datetime(5)" == data_type
            or "datetime(6)" == data_type
        ):
            # CH datetime range is not the same as MySQL https://clickhouse.com/docs/en/sql-reference/data-types/datetime64/
            ...
            "timestamp" == data_type
            or "timestamp(1)" == data_type
            or "timestamp(2)" == data_type
            or "timestamp(3)" == data_type
            or "timestamp(4)" == data_type
            or "timestamp(5)" == data_type
            or "timestamp(6)" == data_type
        ):
...
def select_table_statements(
    table, query, select_query, order_by, external_column_types
):
...
def calculate_sql_checksum(table):
    ...
    try:
        ...
        (query, select_query, distributed_by, external_table_types) = (
            get_table_checksum_query(table, conn)
        )
...
def calculate_checksum(mysql_table):
    if args.ignore_tables_regex:
        ...
        if rex_ignore_tables.match(mysql_table):
            logging.info("Ignoring " + mysql_table + " due to ignore_regex_tables")
            return
    statements = []
...
def main():
    ...
    parser.add_argument("--mysql_password", help="MySQL password", required=True)
    parser.add_argument("--mysql_database", help="MySQL database", required=True)
    parser.add_argument("--mysql_port", help="MySQL port", default=3306, required=False)
    ...
    parser.add_argument(
        "--ignore_tables_regex", help="Ignore table regexp", required=False
    )
    parser.add_argument(
        "--no_wc",
        action="store_true",
        default=False,
        help="Use --tables_regex as the table",
        required=False,
    )
    parser.add_argument(
        "--debug_output",
        action="store_true",
        default=False,
        help="Output the raw format to a file called out.txt",
        required=False,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--exclude_columns",
        help="columns exclude",
        nargs="+",
        default=["_sign", "_version"],
    )
    parser.add_argument(
        "--threads", type=int, help="number of parallel threads", default=1
    )
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            for table in tables.fetchall():
                futures.append(
                    executor.submit(calculate_sql_checksum, table["table_name"])
                )

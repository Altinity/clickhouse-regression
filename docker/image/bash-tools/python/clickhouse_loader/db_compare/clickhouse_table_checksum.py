...
def clickhouse_connection(
    host, database="default", user="default", port=9000, password="", secure=False
):
    conn = connect(
        host=host,
        user=user,
...
def get_table_checksum_query(table):
    ...
    checksum_query = (
        "select name, type, if(match(type,'Nullable'),1,0) is_nullable, numeric_scale from system.columns where database='"
        + args.clickhouse_database
        + "' and table = '"
        + table
        + "' and name not in ("
    ...
    for row in rowset:
        ...
        if "timestamp" in data_type:
            ...
            if "Bool" == data_type:
                ...
                select += (
                    "format_decimal(" + column_name + "," + str(numeric_scale) + ")"
                )
    ...
    primary_key_columns = get_primary_key_columns(args.clickhouse_database, table)
...
def calculate_checksum(table):
    ...
    if rowcount == 0:
        ...
        logging.info(
            "Checksum for table {schema}.{table} = d41d8cd98f00b204e9800998ecf8427e count 0".format(
    ...
    (query, select_query, distributed_by, external_table_types) = (
        get_table_checksum_query(table)
    )
...
def main():
  ...
  Compute the table checksum using the same technique as pt-checksum, md5 algorithm.
    ...
    parser.add_argument("--clickhouse_user", help="ClickHouse user", required=True)
    parser.add_argument(
        "--clickhouse_password", help="ClickHouse password", required=True
    )
    parser.add_argument(
        "--clickhouse_database", help="ClickHouse database", required=True
    )
    parser.add_argument(
        "--clickhouse_port", help="ClickHouse port", default=9000, required=False
    )
    parser.add_argument("--secure", help="True or False", default=False, required=False)
    parser.add_argument(
        "--sign_column",
        help="Override sign column, by default its _sign",
        default="_sign",
        required=False,
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Runs wc first to determine the table names from the regex",
        required=False,
    )
    parser.add_argument(
        "--debug_output",

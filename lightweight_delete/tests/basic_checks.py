...
def delete_zero_rows(self, table_name, node=None):
    ...
    with When(f"I delete zero rows from {table_name}"):
        delete(table_name=table_name, condition="id < -1")
...
def delete_small_subset(self, table_name, node=None):
    ...
    count_all = node.query(
        f"SELECT count(*) FROM {table_name} FORMAT TabSeparated"
    ).output
    count_del = node.query(
        f"SELECT count(*) FROM {table_name} WHERE x < 10 FORMAT TabSeparated"
    ).output
    expected_count = str(int(count_all) - int(count_del))

    with When(f"I delete all rows from {table_name}"):
        delete(table_name=table_name, condition="x < 10")
...
def delete_all_rows_from_half_of_parts(self, table_name, node=None):
    ...
    count_del = node.query(
        f"SELECT count(*) FROM {table_name} WHERE id < 5 FORMAT TabSeparated"
    ).output
    expected_count = str(int(count_all) - int(count_del))

    with When(f"I delete all rows from {table_name}"):
        delete(table_name=table_name, condition="id < 5")

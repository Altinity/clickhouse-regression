"""Helpers for running the lightweight delete suite with content-addressed default storage."""
    ...
    <storage_configuration>
        <disks>
            <cas_disk>
                ...
                <server_root_id>lightweight-delete-cas-{replica}</server_root_id>
...
<clickhouse>
    ...
    This file is filled in by lightweight_delete/cas_mode.py when the suite is
    started with the `cas` option and reset back to this placeholder afterwards.
    -->
</clickhouse>
"""
...
def cas_config_path():
    ...
    The `zz_` prefix keeps it last in the config.d merge order so that the
    `default` policy it defines wins over the local one in `storage.xml`. The
    suite creates tables without an explicit `storage_policy`, so overriding the
    default policy is what puts them on CAS.
    """
    return (
...
def check_cas_mode(test):
    """True when lightweight delete was started with --cas."""

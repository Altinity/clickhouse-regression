        ...
        with Example(
            ...
            with table(
                ...
                with And("I decrypt data during query"):
                    output = node.query(
                        f"""SELECT name, decrypt({example_mode}, secret, {example_key}{(", " + example_iv) if example_iv else ""}{(", " + example_aad) if example_aad else ""}) AS secret FROM user_table FORMAT JSONEachRow"""
                    ).output.strip()
    ...
    for mode, key_len, iv_len, aad_len in modes:
        with Example(
            ...
            with table(
                ...
                with And(
                    "I decrypt data",
                    description="using a subquery and get the number of entries that match the plaintext",
                ):
                    output = node.query(
                ...
                with Then(
                    "I should get back the expected result", description=f"{count}"
                ):

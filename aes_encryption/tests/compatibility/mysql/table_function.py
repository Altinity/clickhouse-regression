            ...
            with Example(
                ...
                with table(
                    ...
                    with And("I read raw data using MySQL table function"):
                        output = node.query(
                            f"SELECT id, date, name, hex(secret) AS secret FROM {table_function} FORMAT TabSeparated"
                        )

                    with And("I read decrypted data using MySQL table function"):
                        output = node.query(
                            f"""SELECT hex({func}({example_mode}, secret, {example_key}{(", " + example_iv) if example_iv else ""})) FROM {table_function} FORMAT TabSeparated"""
                        ).output.strip()
            ...
            with Example(
                ...
                with table(
                    ...
                    with And("I read decrypted data using MySQL database engine"):
                        output = node.query(
                            f"""SELECT hex(aes_decrypt_mysql({example_mode}, secret, {example_key}{(", " + example_iv) if example_iv else ""})) FROM {table_function} FORMAT TabSeparated"""
                        ).output.strip()
                    ...
                    with And(
                        ...
                        expected_raw_data = node.query(
                            f"SELECT hex(secret) AS secret FROM {table_function} FORMAT TabSeparated"
                        ).output.strip()

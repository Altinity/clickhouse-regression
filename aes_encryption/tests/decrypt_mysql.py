...
def aes_decrypt_mysql(
    ...
    if compare:
        sql = f"{compare} = {sql}"
    sql = f"SELECT {sql} FORMAT TabSeparated"
...
def invalid_parameters(self):
    ...
    with Example("bad iv type - UInt8"):
        ...
        if check_clickhouse_version("<24.7")(self):
            ...
            message = "DB::Exception: A value of illegal type was provided as 4th argument 'IV' to function 'aes_decrypt_mysql'."
        aes_decrypt_mysql(
            ciphertext=ciphertext,
            key="'0123456789123456'",
            mode="'aes-128-cbc'",
    ...
    with Example(
        "invalid mode value",
        requirements=[
            RQ_SRS008_AES_MySQL_Decrypt_Function_Parameters_Mode_Value_Invalid("1.0")
        ],
    ):
...
def key_or_iv_length_for_mode(self, mode, key_len, iv_len):
    ...
    with When("key is too long"):
        if "ecb" in mode or "cbc" in mode:
            ...
            aes_decrypt_mysql(
                ciphertext=ciphertext, key=f"'{key[:key_len+1]}'", mode=mode, cast="hex"
            )
...
@TestScenario
@Requirements(RQ_SRS008_AES_MySQL_Decrypt_Function_Parameters_Key("1.0"))
...
@TestScenario
@Requirements(RQ_SRS008_AES_MySQL_Decrypt_Function_Parameters_ReturnValue("1.0"))
def return_value(self):
    ...
    with When("I get type of the return value"):
        sql = (
            "SELECT toTypeName(aes_decrypt_mysql("
            + mode
            + ","
            + ciphertext
            + ","
    ...
    with When("I get the return value"):
        aes_decrypt_mysql(
            ciphertext=ciphertext, key=key, mode=mode, iv=iv, message="hello there"
        )
...
def syntax(self):
    ...
    sql = f"SELECT aes_decrypt_mysql('aes-128-ofb', unhex('{ciphertext}'), '0123456789123456', '0123456789123456') FORMAT TabSeparated"
...
@TestFeature
@Name("decrypt_mysql")
@Requirements(RQ_SRS008_AES_MySQL_Decrypt_Function("1.0"))

    ...
    <storage_configuration>
        <disks>
            <cas_disk>
                ...
                <server_root_id>atomic-insert-cas-{replica}</server_root_id>
...
<clickhouse>
    ...
    with the `cas` option and reset back to this placeholder afterwards.
    -->
</clickhouse>
"""
...
def cas_config_path():
    ...
    `default` policy wins. Tables created without an explicit `storage_policy`
    (or via helpers.tables) then land on CAS.
    """
    return (
...
def check_cas_mode(test):
    """True when atomic insert was started with --cas."""

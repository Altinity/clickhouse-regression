                    ...
                    *[
                    ...
                    with rbac_roles(f"new_role0_{uid}") as new_roles:
                        ...
                        with And("I try to grant new role to the cached LDAP user"):
                            node.query(
                                f"GRANT {new_roles[0]} TO {users[0]['username']}",
                                exitcode=exitcode,
                                message=message.format(user=users[0]["username"]),
                            )
                        ...
                        with And("I try to grant new role to the non-cached LDAP user"):
                            node.query(
                                f"GRANT {new_roles[0]} TO {users[1]['username']}",
                                exitcode=exitcode,
                                message=message.format(user=users[1]["username"]),
                            )


@TestScenario
@Requirements(RQ_SRS_009_LDAP_ExternalUserDirectory_Role_NewPrivilege("1.0"))
...
@TestScenario
@Requirements(RQ_SRS_009_LDAP_ExternalUserDirectory_Role_RemovedPrivilege("1.0"))
            ...
            with ldap_external_user_directory(server=server, roles=roles, restart=True):
                with ldap_users(
                    *[
                    ...
                    with When(
                        f"I revoke select privilege from all the roles assigned to LDAP users"
                    ):
...
@TestScenario
@Requirements(RQ_SRS_009_LDAP_ExternalUserDirectory_Role_Removed("2.0"))
...
@TestScenario
@Requirements(RQ_SRS_009_LDAP_ExternalUserDirectory_Role_Removed_Privileges("1.0"))
            ...
            with ldap_external_user_directory(server=server, roles=roles, restart=True):
                with ldap_users(
                    *[
                    ...
                    with And(
                        f"I try to relogin and expect that cached LDAP user can login "
...
@TestScenario
@Requirements(RQ_SRS_009_LDAP_ExternalUserDirectory_Role_Readded_Privileges("1.0"))
            ...
            with ldap_external_user_directory(server=server, roles=roles, restart=True):
                with ldap_users(
                    *[
                    ...
                    with And(
                        "I try to login using non-cached LDAP expect it to also work again and expect"
...
@TestScenario
@Requirements(RQ_SRS_009_LDAP_ExternalUserDirectory_Role_NotPresent_Added("1.0"))
                ...
                *[
                ...
                with When(f"I login using clickhouse-client"):
                    with self.context.cluster.shell(node=node.name) as shell:
                        with shell(
                            f"TERM=dumb clickhouse client --user {users[0]['username']} --password {users[0]['password']} | tee",
                            asynchronous=True,
                            name="client",
                        ) as client:

...
def login_and_execute_query(
    self, username, password, exitcode=None, message=None, steps=True
):
...
def add_user_to_ldap_and_login(
    ...
    with ldap_user(**user) as user:
        ...
        with ldap_authenticated_users(
            ch_user, config_file=f"ldap_users_{getuid()}.xml", restart=True, rbac=rbac
        ):
...
def parallel_login(self, server, user_count=10, timeout=300, rbac=False):
    ...
    with ldap_users(*users):
        with ldap_authenticated_users(
            *[{"username": user["cn"], "server": server} for user in users], rbac=rbac
        ):
...
def login_after_ldap_server_is_restarted(self, server, timeout=300, rbac=False):
    ...
    try:
        ...
        with ldap_authenticated_users(
            {"username": user["cn"], "server": server}, rbac=rbac
        ):
            login_and_execute_query(username=user["cn"], password=user["userpassword"])
...
@TestScenario
@Requirements(RQ_SRS_007_LDAP_Authentication_Valid("1.0"))
...
@TestScenario
@Requirements(RQ_SRS_007_LDAP_Authentication_Invalid("1.0"))
...
@Requirements(RQ_SRS_007_LDAP_Configuration_Server_VerificationCooldown_Default("1.0"))
...
@TestScenario
@Tags("verification_cooldown")
@Requirements(RQ_SRS_007_LDAP_Configuration_Server_VerificationCooldown("1.0"))
...
def check_verification_cooldown_reset_on_core_server_parameter_change(
    ...
    with ldap_users(*users) as users:
        with ldap_servers(servers, restart=True):
            with ldap_authenticated_users(
                *[{"username": user["cn"], "server": server} for user in users]
            ):
                with When("I login and execute a query"):
                    for user in users:
...
@Requirements(RQ_SRS_007_LDAP_Authentication_Mechanism_NamePassword("1.0"))
@Examples(
    "rbac",
    [(False,), (True, Requirements(RQ_SRS_007_LDAP_Configuration_User_RBAC("1.0")))],
)

...
@Examples(
    ...
    [
        ["MergeTree()"],
        # ["ReplicatedMergeTree('/clickhouse/altering_replicated_mt', '1')]
...
def scenario(self, engine):
    ...
    try:
        ...
        with And("I insert 4 values"):
            ...
            node.query(f"INSERT INTO {table_name} VALUES(toDate('2019-03-16'), 66)")
            node.query(f"INSERT INTO {table_name} VALUES(toDate('2019-04-10'), 42)")
            node.query(f"INSERT INTO {table_name} VALUES(toDate('2019-04-11'), 43)")
        ...
        with When("I get the first part from system.parts"):
            ...
            with And("I try to move first part to 'external' volume"):
                time.sleep(1)
                node.query(
                    f"ALTER TABLE {table_name} MOVE PART '{first_part}' TO VOLUME 'external'"
                )
        ...
        with When("I move the first part to 'jbod1' disk"):
            time.sleep(1)
            node.query(
                f"ALTER TABLE {table_name} MOVE PART '{first_part}' TO DISK 'jbod1'"
            )
        ...
        with When("I move partition 201904 to 'external' volume"):
            time.sleep(1)
            node.query(
                f"ALTER TABLE {table_name} MOVE PARTITION 201904 TO VOLUME 'external'"
            )
        ...
        with When("I move partition 201904 to disk 'jbod2'"):
            time.sleep(1)
            node.query(
                f"ALTER TABLE {table_name} MOVE PARTITION 201904 TO DISK 'jbod2'"
            )

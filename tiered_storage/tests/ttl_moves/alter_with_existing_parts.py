...
@Examples(
    ...
    [
        ...
        (
            "replicated_mt_test_alter_with_existing_parts",
            "ReplicatedMergeTree('/clickhouse/replicated_test_alter_with_existing_parts', '1')",
        ),
    ],
    "%-21s | %-20s",
)
            ...
            try:
                ...
                with And("I wait until second TTL expression triggers"):
                    ...
                    with Then("again number of rows should match"):
                        for attempt in retries(timeout=60, delay=15):
                            with attempt:
                                r = node.query(
                                    f"SELECT count() FROM {name} FORMAT TabSeparated"
                                ).output.strip()

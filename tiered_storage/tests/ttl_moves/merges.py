...
def scenario(self, name, engine):
    ...
    for move_to, positive in product(("DISK", "VOLUME"), (True, False)):
        with Check(f"parts should {'be' if positive else 'not be'} moved to {move_to}"):
            try:
                ...
                with And("I wait for 1 sec for merge with move to complete"):
                    time.sleep(1)

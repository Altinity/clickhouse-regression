        ...
        with Check(f"parts should {'be' if positive else 'not be'} moved"):
            try:
                ...
                with Given(
                    ...
                    try:
                        wait_expire_1 = 10
                        time_1 = time.time() + wait_expire_1 + (0 if positive else 300)
                        ...
                        with And("I get used disks for the second table"):
                            ...
                            with Then(f"check that all parts are on the first disk"):
                        ...
                        with And("I get used disks for the second table after merge"):
                            used_disks = get_used_disks_for_table(node, name)
                            with Then(
                        ...
                        with And("then I drop temporary table to free up second disk"):
                        ...
                        with Then("I again check that number of rows did not change"):
                            r = node.query(
                                f"SELECT count() FROM {name} FORMAT TabSeparated"
                            ).output.strip()
                            assert r == "12", error()

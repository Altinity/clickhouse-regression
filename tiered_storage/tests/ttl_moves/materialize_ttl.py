...
def scenario(self, name, engine, partitions_to_materialize):
        ...
        with Check(f"parts should {'be' if positive else 'not be'} moved"):
            try:
                ...
                with When(
                    ...
                    with When("I get used disks for the table", description="20 sec"):
                        time.sleep(20)
                            ...
                            with attempt:
                                ...
                                if not positive:
                                    ...
                                    if not partitions_to_materialize or set(
                                        ...
                                        with Then(f"all parts should have been moved"):
                                            assert set(used_disks) == {
                                                "external"
                                            }, error()
                                    else:
                                        with Then("some parts should have been moved"):
                        ...
                        with attempt:
                            if not positive:
                                ...
                                if not partitions_to_materialize or set(
                                    ...
                                    with Then(f"all rows should have been deleted"):
                                        r = node.query(
                                            f"SELECT count() FROM {name} FORMAT TabSeparated"
                                        ).output.strip()
                                        assert r == "0", error()
                                else:
                                    with Then(f"some rows should have been deleted"):
                                        r = node.query(
                                            f"SELECT count() FROM {name} FORMAT TabSeparated"
                                        ).output.strip()
                                        assert (

...
def scenario(self, name, engine):
    ...
    for positive in (True, False):
        with Check(f"existing parts should {'be' if positive else 'not be'} moved"):
            try:
                with Given("table"):
                    node.query(
                        f"""
                ...
                with When("I create 60 partitions with one for each day"):
                    ...
                    for p in range(60):
                        ...
                        values = ",".join(data)
                        node.query(f"INSERT INTO {name} (d1, t1, s1) VALUES {values}")
                ...
                with When("I change storage policy to include new external volume"):
                    node.query(
                        f"ALTER TABLE {name} MODIFY SETTING storage_policy='jbods_with_external'"
                    )

                with And("I update table settings to include TTL to move old data"):
                    node.query(
                ...
                with And("I get used disks for the table after table is optimized"):
                    used_disks = set(get_used_disks_for_table(node, name))
                    with Then(

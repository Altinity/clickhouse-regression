...
@Examples(
    ...
    [
        ...
        (
            "replicated_mt_alter_column_in_ttl",
            "ReplicatedMergeTree('/clickhouse/replicated_alter_column_in_ttl', '1')",
        ),
    ],
    "%-31s | %-20s",
)
    ...
    try:
        with Given("I create table"):
            ...
            with And("some data in the table"):
                for i in range(4):
                    node.query(f"INSERT INTO {name} VALUES ({date(0)},1,'fast{i}')")

        with When("I try to drop column"):
            exitcode = 47
            message = "Exception: Missing columns: 'ttl_days'"
            node.query(f"ALTER TABLE {name} DROP COLUMN ttl_days", message, exitcode)

            message = "Exception: Missing columns: 'date'"
            node.query(f"ALTER TABLE {name} DROP COLUMN date", message, exitcode)
        ...
        with When("I try to add column"):
            ...
            with And("I insert a row that would be remove with new TTL expressions"):
                node.query(f"INSERT INTO {name} VALUES ({date(-2)},1,0,'delete')")

            with Then("I check the row was removed"):
                if hasattr(cluster, "with_minio") and hasattr(cluster, "with_s3amazon"):

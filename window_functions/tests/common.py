...
def invalid_expression_error():
    if check_clickhouse_version(">=24.9")(current()):
        return (47, "DB::Exception: Unknown expression identifier `foo`")
    elif is_with_analyzer(node=current().context.node):
        return (47, "DB::Exception: Unknown expression or function identifier 'foo'")
    else:
        return (47, "Exception: Missing columns: 'foo'")

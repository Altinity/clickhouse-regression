...
@TestScenario
@Requirements(RQ_SRS_019_ClickHouse_WindowFunctions_OrderClause_MultipleExprs("1.0"))
...
def missing_expr_error(self):
    ...
    self.context.node.query(
        "SELECT sum(number) OVER (ORDER BY) FROM numbers(1,3)",
        exitcode=exitcode,
        message=message,
    )
...
def invalid_expr_error(self):
    ...
    self.context.node.query(
        "SELECT sum(number) OVER (ORDER BY foo) FROM numbers(1,3)",
        exitcode=exitcode,
        message=message,
    )
...
def by_a_non_integer_constant(self):
    ...
    execute_query(
        "SELECT rank() OVER (ORDER BY length('abc')) AS rank", expected=expected
    )


@TestFeature
@Name("order clause")
@Requirements(RQ_SRS_019_ClickHouse_WindowFunctions_OrderClause("1.0"))
def feature(self):

...
def different_nodes_same_destination(self, cluster, node1, node2):
    ...
    with Given("I create an empty source table and empty S3 table"):
        ...
        s3_table_name = create_s3_table(table_name="s3", create_new_bucket=True, cluster=cluster)
    ...
    with And("I read data from all tables on both nodes"):
        ...
        destination_data1 = select_all_ordered(
            table_name=s3_table_name, node=node1
        )
        destination_data2 = select_all_ordered(
            table_name=s3_table_name, node=node2
        )

    with Then("Destination data should be comprised of data from both sources, and identical on both nodes"):

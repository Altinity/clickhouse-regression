...
@Examples(
    ...
    [
        ...
        ("{759040,547776,167687,283359}", 60, True, Name("nums")),
        ("{759040,547776,167687,abc,283359}", 60, True, Name("nums one invalid")),
        ...
        ("{abc,efg,hij}", 10, False, Name("nums no match")),
...
def wildcard(self, wildcard, expected_time, expect_result):
    ...
    for i in range(1, 3):
        with Then(f"""I query S3 using the wildcard '{wildcard}'"""):
            ...
            assert r.output.strip().replace(".", "").isdigit() == expect_result, error()

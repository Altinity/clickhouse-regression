...
def cascade_mv_definer_mv_definer_mv_definer(
    ...
    source table -> materialized view with definer ->
    materialized view with definer -> materialized view with definer
    """
    ...
    with And("I grant SELECT privilege on third materialized view to new user"):
        grant_privilege(privileges=["SELECT"], object=view_name_3, user=new_user)

    with And("I try to select from third materialized view with new user"):
        exitcode, message = 241, f"Exception:"
        if (
    ...
    with And("I grant INSERT privilege on third materialized view to new user"):
        grant_privilege(privileges=["INSERT"], object=view_name_3, user=new_user)

    with And("I try to insert into third materialized view with new user"):
        exitcode, message = 241, f"Exception:"
...
def cascade_mv_mv_definer_mv_definer(
    ...
    """
    Test privileges for different operations on cascading view:
    source table -> materialized view without sql security ->
    materialized view with definer -> materialized view with definer
    """
    node = self.context.node
...
def check_cascade_mv_definer_mv_definer_mv_definer(self):
    ...
    source table -> materialized view with definer -> materialized view with definer ->
    materialized view with definer.
    """
    ...
    with Pool(7) as executor:
        for (
            ...
            test_name = f"cascade_mv_definer_mv_definer_mv_definer_{user_source_privileges}_{definer_one_source_privileges}_{definer_one_target_one_privileges}_{definer_two_target_one_privileges}_{definer_two_target_two_privileges}_{definer_three_target_two_privileges}_{definer_three_target_three_privileges}_{grant_privilege.__name__}"
...
@TestScenario
def check_cascade_mv_mv_definer_mv_definer(self, node="clickhouse1"):
    """
    Run test for all possible combinations of privileges for cascading view:
    source table -> materialized view -> materialized view with definer ->
    materialized view with definer.
    """
    ...
    with Pool(7) as executor:
        for (
            ...
            test_name = f"cascade_mv_mv_definer_mv_definer_{user_source_privileges}_{definer_two_target_one_privileges}_{definer_two_target_two_privileges}_{definer_three_target_two_privileges}_{definer_three_target_three_privileges}_{grant_privilege.__name__}"
            test_name = test_name.replace("[", "_").replace("]", "_").replace(" ", "")
            Scenario(
                test_name,
                description=f"""

...
def cascade_mv_definer_mv_definer_mv_definer(
    ...
    with And(
        "new user to test privileges for different operations with cascading view"
    ):
...
def check_cascade_mv_definer_mv_definer_mv_definer(self):
    ...
    with And(
        ...
        with By(
            "definer privileges for the user executing queries for the source table of the first MV"
        ):

...
def create_without_create_view_privilege(self, node=None):
    ...
    with user(node, f"{user_name}"):
        with When(
            "I try to create a live view without CREATE VIEW privilege as the user"
        ):
            node.query(f"DROP VIEW IF EXISTS {view_name}")
            node.query(
                f"CREATE LIVE VIEW {view_name} AS SELECT 1",
...
def create_without_source_table_privilege(self, node=None):
    ...
    with table(node, f"{table_name}"):
        with user(node, f"{user_name}"):
            ...
            with Then(
                "I try to create a live view without select privilege on the table"
            ):
                node.query(
...
def create_with_join_subquery_privilege_granted_directly_or_via_role(self, node=None):
    ...
    with user(node, f"{user_name}"):
        Scenario(
            test=create_with_join_subquery,
            name="create with join subquery, privilege granted directly",
        )(grant_target_name=user_name, user_name=user_name)
    ...
    with user(node, f"{user_name}"), role(node, f"{role_name}"):
        ...
        Scenario(
            test=create_with_join_subquery,
            name="create with join subquery, privilege granted through a role",
        )(grant_target_name=role_name, user_name=user_name)
    ...
    with table(node, f"{table0_name},{table1_name},{table2_name},{table3_name}"):
        try:
            ...
            with When("I grant select on all tables"):
                with grant_select_on_table(
                    node,
                    max(permutations(table_count=4)) + 1,
                    grant_target_name,
                    table0_name,
                    table1_name,
                    table2_name,
...
def create_with_nested_views(self, grant_target_name, user_name, node=None):
    ...
    with table(node, f"{table0_name},{table1_name},{table2_name}"):
        try:
            ...
            for permutation in (
                [0, 1, 2, 3, 10, 14, 15, 26, 27, 30],
                permutations(table_count=5),
            )[self.context.stress]:
                with grant_select_on_table(
                    node,
                    permutation,
...
def select(self, node=None):
    ...
    Scenario(
        run=select_with_join_subquery_privilege_granted_directly_or_via_role,
        setup=instrument_clickhouse_server_log,
    )
    Scenario(
...
def select_with_select_privilege_revoked_directly_or_from_role(self, node=None):
    ...
    with user(node, f"{user_name}"):
        Scenario(
            test=select_with_revoked_select_privilege,
    ...
    with table(node, f"{table0_name},{table1_name},{table2_name}"):
        try:
            with Given("I have a view with a subquery"):
                node.query(f"DROP VIEW IF EXISTS {view_name}")
                node.query(
                    f"CREATE LIVE VIEW {view_name} AS SELECT * FROM {table0_name} WHERE y IN (SELECT y FROM {table1_name} WHERE y IN (SELECT y FROM {table2_name} WHERE y<2))"
                )
    ...
    with table(node, f"{table0_name},{table1_name}"):
        try:
            with Given("I have a view with a JOIN statement"):
                node.query(f"DROP VIEW IF EXISTS {view_name}")
                node.query(
                    f"CREATE LIVE VIEW {view_name} AS SELECT * FROM {table0_name} JOIN {table1_name} USING d"
                )
...
def select_with_join_subquery_privilege_granted_directly_or_via_role(self, node=None):
    ...
    with user(node, f"{user_name}"):
        Scenario(
            test=select_with_join_subquery,
            name="select with join subquery, privilege granted directly",
        )(grant_target_name=user_name, user_name=user_name)
    ...
    with user(node, f"{user_name}"), role(node, f"{role_name}"):
        ...
        Scenario(
            test=select_with_join_subquery,
            name="select with join subquery, privilege granted through a role",
        )(grant_target_name=role_name, user_name=user_name)
    ...
    with table(node, f"{table0_name},{table1_name},{table2_name},{table3_name}"):
        try:
            with Given("I have a view"):
                node.query(f"DROP VIEW IF EXISTS {view_name}")
                node.query(
                    f"CREATE LIVE VIEW {view_name} AS SELECT y FROM {table0_name} WHERE y IN (SELECT y FROM {table1_name} WHERE y IN (SELECT y FROM {table2_name} WHERE y<2)) JOIN {table3_name} USING y"
                )
...
def select_with_nested_views(self, grant_target_name, user_name, node=None):
    ...
    with table(node, f"{table0_name},{table1_name},{table2_name}"):
        try:
            with Given("I have some views"):
                node.query(
                    f"CREATE LIVE VIEW {view0_name} AS SELECT y FROM {table0_name}"
                )
                node.query(
                    f"CREATE LIVE VIEW {view1_name} AS SELECT y FROM {view0_name} WHERE y IN (SELECT y FROM {table1_name} WHERE y<2)"
                )
                node.query(
                    f"CREATE LIVE VIEW {view2_name} AS SELECT y FROM {view1_name} JOIN {table2_name} USING y"
                )

            with Then("I attempt to select from a view as the user"):
                node.query(
                    select_view_query.format(view2_name=view2_name),
                    settings=[("user", f"{user_name}")],
                    exitcode=exitcode,
                    message=message,
                )
            ...
            for permutation in (
                [0, 1, 3, 5, 7, 21, 29, 31, 53, 55, 61],
                permutations(table_count=6),
            )[self.context.stress]:
                with grant_select_on_table(
                    node,
                    permutation,
            ...
            with When("I grant select on all views"):
                with grant_select_on_table(
                    node,
                    max(permutations(table_count=6)) + 1,
                    grant_target_name,
                    view0_name,
                    view1_name,
                    view2_name,
...
def refresh(self, node=None):
    ...
    Scenario(
        run=refresh_with_privilege_granted_directly_or_via_role,
        setup=instrument_clickhouse_server_log,
    )
    Scenario(
        run=refresh_with_privilege_revoked_directly_or_from_role,
        setup=instrument_clickhouse_server_log,
    )
...
def refresh_with_privilege_granted_directly_or_via_role(self, node=None):
    ...
    with user(node, f"{user_name}"):
        Scenario(
            test=refresh_with_privilege, name="refresh privilege granted directly"
        )(grant_target_name=user_name, user_name=user_name)
    ...
    with user(node, f"{user_name}"), role(node, f"{role_name}"):
        with When("I grant the role to the user"):
            node.query(f"GRANT {role_name} TO {user_name}")
        Scenario(
            test=refresh_with_privilege, name="refresh privilege revoked from a role"
        )(grant_target_name=role_name, user_name=user_name)
...
def refresh_with_privilege(self, grant_target_name, user_name, node=None):
    ...
    try:
        ...
        with When("I grant REFRESH privilege"):
            node.query(
                f"GRANT ALTER VIEW REFRESH ON {view_name} TO {grant_target_name}"
            )

        with Then("I attempt to refresh as the user"):
            node.query(
                f"ALTER LIVE VIEW {view_name} REFRESH",
                settings=[("user", f"{user_name}")],
            )
...
def refresh_with_privilege_revoked_directly_or_from_role(self, node=None):
    ...
    with user(node, f"{user_name}"):
        Scenario(
            test=refresh_with_revoked_privilege,
            name="refresh privilege revoked directly",
        )(grant_target_name=user_name, user_name=user_name)
    ...
    with user(node, f"{user_name}"), role(node, f"{role_name}"):
        ...
        Scenario(
            test=refresh_with_revoked_privilege,
            name="refresh privilege revoked from a role",
        )(grant_target_name=role_name, user_name=user_name)
...
def refresh_with_revoked_privilege(self, grant_target_name, user_name, node=None):
    ...
    try:
        ...
        with And("I revoke REFRESH privilege"):
            node.query(
                f"REVOKE ALTER VIEW REFRESH ON {view_name} FROM {grant_target_name}"
            )

...
@Examples(
    ...
    [
        (
            ...
            False,
            False,
            Requirements(RQ_SRS_006_RBAC_Grant_Privilege_DictGet("1.0")),
        ),
        (
            "INTROSPECTION",
            ("*.*",),
            False,
            True,
            Requirements(RQ_SRS_006_RBAC_Grant_Privilege_Introspection("1.0")),
        ),
        (
            "SELECT",
            ("db0.table0", "db0.*", "*.*", "tb0", "*"),
            True,
            False,
            Requirements(RQ_SRS_006_RBAC_Grant_Privilege_Select("1.0")),
        ),
        (
            "INSERT",
            ("db0.table0", "db0.*", "*.*", "tb0", "*"),
            True,
            False,
            Requirements(RQ_SRS_006_RBAC_Grant_Privilege_Insert("1.0")),
        ),
        (
            "ALTER",
            ("db0.table0", "db0.*", "*.*", "tb0", "*"),
            False,
            False,
            Requirements(RQ_SRS_006_RBAC_Grant_Privilege_Alter("1.0")),
        ),
        (
            "CREATE",
            ("db0.table0", "db0.*", "*.*", "tb0", "*"),
            False,
            False,
            Requirements(RQ_SRS_006_RBAC_Grant_Privilege_Create("1.0")),
        ),
        (
            "DROP",
            ("db0.table0", "db0.*", "*.*", "tb0", "*"),
            False,
            False,
            Requirements(RQ_SRS_006_RBAC_Grant_Privilege_Drop("1.0")),
        ),
        (
            "TRUNCATE",
            ("db0.table0", "db0.*", "*.*", "tb0", "*"),
            False,
            False,
            Requirements(RQ_SRS_006_RBAC_Grant_Privilege_Truncate("1.0")),
        ),
        (
            "OPTIMIZE",
            ("db0.table0", "db0.*", "*.*", "tb0", "*"),
            False,
            False,
            Requirements(RQ_SRS_006_RBAC_Grant_Privilege_Optimize("1.0")),
        ),
        (
            "SHOW",
            ("db0.table0", "db0.*", "*.*", "tb0", "*"),
            True,
            False,
            Requirements(RQ_SRS_006_RBAC_Grant_Privilege_Show("1.0")),
        ),
        (
            "KILL QUERY",
            ("*.*",),
            False,
            False,
            Requirements(RQ_SRS_006_RBAC_Grant_Privilege_KillQuery("1.0")),
        ),
        (
            "ACCESS MANAGEMENT",
            ("*.*",),
            False,
            False,
            Requirements(RQ_SRS_006_RBAC_Grant_Privilege_AccessManagement("1.0")),
        ),
        (
            "SYSTEM",
            ("db0.table0", "db0.*", "*.*", "tb0", "*"),
            False,
            False,
            Requirements(RQ_SRS_006_RBAC_Grant_Privilege_System("1.0")),
        ),
        (
            "SOURCES",
            ("*.*",),
            False,
            False,
            Requirements(RQ_SRS_006_RBAC_Grant_Privilege_Sources("1.0")),
        ),
        (
            "ALL",
            ("*.*",),
            True,
            True,
            Requirements(RQ_SRS_006_RBAC_Grant_Privilege_All("1.0")),
        ),
        (
            "ALL PRIVILEGES",
            ("*.*",),
        ...
        with When(f"I grant {privilege} privilege to user on {on_}"):
            with setup(node):
                ...
                with When("I grant privilege without grant option"):
                    node.query(
                        f"GRANT {privilege} ON {on_} TO user0", settings=settings
                    )
                with When("I grant privilege with grant option"):
                    node.query(
                        f"GRANT {privilege} ON {on_} TO user1 WITH GRANT OPTION",
                        settings=settings,
                    )
                ...
                if allow_column and ("*" not in on_):
                    ...
                    with When("I grant privilege with columns"):
                        node.query(
                            f"GRANT {privilege}(x) ON {on_} TO user0", settings=settings
                        )
...
def feature(self, node="clickhouse1"):
    ...
    with Scenario(
        "I grant privilege to role that does not exist",
    ...
    with Scenario(
        "I grant privilege to multiple users and roles",
        requirements=[
    ...
    with Scenario(
        "I grant privilege to current user",
        requirements=[
    ...
    with Scenario(
        "I grant privilege NONE to default user, throws exception",
        requirements=[
            RQ_SRS_006_RBAC_Grant_Privilege_ToCurrentUser("1.0"),
            RQ_SRS_006_RBAC_Grant_Privilege_None("1.0"),
        ],
    ...
    with Scenario(
        "I grant privilege with grant option",
        requirements=[

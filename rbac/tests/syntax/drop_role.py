...
def feature(self, node="clickhouse1"):
    ...
    with Scenario(
        "I drop role with no options", requirements=[RQ_SRS_006_RBAC_Role_Drop("1.0")]
    ):
        with setup("role0"):
    ...
    with Scenario(
        "I drop role that doesn't exist, throws exception",
        requirements=[RQ_SRS_006_RBAC_Role_Drop("1.0")],
    ):
        role = "role0"
        cleanup_role(role)
    ...
    with Scenario(
        "I drop multiple roles", requirements=[RQ_SRS_006_RBAC_Role_Drop("1.0")]
    ):
    ...
    with Scenario(
        "I drop role that does not exist, using if exists",
        requirements=[RQ_SRS_006_RBAC_Role_Drop_IfExists("1.0")],
    ):
        with When("I drop role if exists"):
            node.query("DROP ROLE IF EXISTS role3")

    with Scenario(
        "I drop multiple roles where one does not exist",
        requirements=[RQ_SRS_006_RBAC_Role_Drop_IfExists("1.0")],
    ):
        with setup("role5"):
    ...
    with Scenario(
        "I drop multiple roles where both do not exist",
        requirements=[RQ_SRS_006_RBAC_Role_Drop_IfExists("1.0")],
    ):

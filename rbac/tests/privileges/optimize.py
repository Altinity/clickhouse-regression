...
def privilege_granted_directly_or_via_role(self, table_type, node=None):
    ...
    with Suite("user with privilege via role"):
        with user(node, user_name), role(node, role_name):
            ...
            with And(
                f"I run checks that {user_name} with {role_name} is only able to execute OPTIMIZE with required privileges"
            ):
                privilege_check(
    ...
    with Scenario("user with privilege"):
        ...
        with table(node, table_name, table_type):
            ...
            with Then("I attempt to optimize a table"):
                node.query(
                    f"OPTIMIZE TABLE {table_name}", settings=[("user", user_name)]
                )
    ...
    with Scenario("execute on cluster"):
        ...
        try:
            with Given("I have a table on a cluster"):
                node.query(
                    f"CREATE TABLE {table_name} ON CLUSTER sharded_cluster (d DATE, a String, b UInt8, x String, y Int8) ENGINE = MergeTree() PARTITION BY y ORDER BY d"
                )

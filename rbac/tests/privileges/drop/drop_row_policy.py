        ...
        Suite(
            ...
            examples=Examples(
                ...
                [
                    ...
                    for row in drop_row_policy.examples
                ],
    ...
    with Scenario("DROP ROW POLICY without privilege"):
        ...
        try:
            ...
            with Then("I check the user can't drop a row policy"):
                node.query(
                    f"DROP ROW POLICY {drop_row_policy_name} ON {table_name}",
            ...
            with Finally("I drop the row policy"):
                node.query(
                    f"DROP ROW POLICY IF EXISTS {drop_row_policy_name} ON {table_name}"
                )
    ...
    with Scenario("DROP ROW POLICY on cluster"):
        ...
        try:
            with Given("I have a row policy on a cluster"):
                node.query(
                    f"CREATE ROW POLICY {drop_row_policy_name} ON CLUSTER sharded_cluster ON {table_name}"
                )
            ...
            with Finally("I drop the user"):
                node.query(
                    f"DROP ROW POLICY IF EXISTS {drop_row_policy_name} ON CLUSTER sharded_cluster ON {table_name}"
                )
...
def drop_on_cluster(self, node=None):
    ...
    try:
        ...
        with When("I drop the row policy on cluster"):
            node.query(
                f"DROP ROW POLICY {pol_name} ON {table_name} ON CLUSTER sharded_cluster"
            )

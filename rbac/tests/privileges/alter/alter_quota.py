        ...
        Suite(
            ...
            examples=Examples(
                ...
                [
                    ...
                    for row in alter_quota.examples
                ],
    ...
    with Scenario("ALTER QUOTA without privilege"):
        ...
        with quota(node, alter_quota_name):
            ...
            with Then("I check the user can't alter a quota"):
                node.query(
                    f"ALTER QUOTA {alter_quota_name}",
    ...
    with Scenario("ALTER QUOTA on cluster"):
        ...
        try:
            with Given("I have a quota on a cluster"):
                node.query(
                    f"CREATE QUOTA {alter_quota_name} ON CLUSTER sharded_cluster"
                )
            ...
            with Then("I check the user can alter a quota"):
                if check_clickhouse_version(">=24.4")(self) and privilege != "ALL":
                    node.query(
                        f"ALTER QUOTA {alter_quota_name} ON CLUSTER sharded_cluster",
                        settings=[("user", f"{user_name}")],
                        exitcode=exitcode,
                        message=message,
                    )
            ...
            with Finally("I drop the quota"):
                node.query(
                    f"DROP QUOTA IF EXISTS {alter_quota_name} ON CLUSTER sharded_cluster"
                )

    ...
    with Scenario("ALTER SETTINGS PROFILE without privilege"):
        ...
        with settings_profile(node, alter_settings_profile_name):
            ...
            with Then("I check the user can't alter a settings_profile"):
                node.query(
                    f"ALTER SETTINGS PROFILE {alter_settings_profile_name}",
    ...
    with Scenario("ALTER SETTINGS PROFILE on cluster"):
        ...
        try:
            with Given("I have a settings_profile on a cluster"):
                node.query(
                    f"CREATE SETTINGS PROFILE {alter_settings_profile_name} ON CLUSTER sharded_cluster"
                )
            ...
            with Then("I check the user can alter a settings_profile"):
                if check_clickhouse_version(">=24.4")(self) and privilege != "ALL":
                    node.query(
                        f"ALTER SETTINGS PROFILE {alter_settings_profile_name} ON CLUSTER sharded_cluster",
                        settings=[("user", f"{user_name}")],
                        exitcode=exitcode,
                        message=message,
                    )
            ...
            with And("I grant cluster privilege"):
                if check_clickhouse_version(">=24.4")(self) and privilege != "ALL":
            ...
            with Finally("I drop the settings_profile"):
                node.query(
                    f"DROP SETTINGS PROFILE IF EXISTS {alter_settings_profile_name} ON CLUSTER sharded_cluster"
                )
...
def feature(self, node="clickhouse1"):
    ...
    Suite(
        run=alter_settings_profile_granted_directly,
        setup=instrument_clickhouse_server_log,
    )
    Suite(
        run=alter_settings_profile_granted_via_role,
        setup=instrument_clickhouse_server_log,
    )

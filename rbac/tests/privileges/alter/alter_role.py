        ...
        Suite(
            ...
            examples=Examples(
                ...
                [
                    ...
                    for row in alter_role.examples
                ],
    ...
    with Scenario("ALTER ROLE with privilege"):
        ...
        with role(node, alter_role_name):
            ...
            with Then("I check the user can alter a role"):
                node.query(
                    f"ALTER ROLE {alter_role_name}", settings=[("user", f"{user_name}")]
                )
        ...
        try:
            ...
            with Then("I check the user can alter a role"):
                if check_clickhouse_version(">=24.4")(self) and privilege != "ALL":
                    node.query(
                        f"ALTER ROLE {alter_role_name} ON CLUSTER sharded_cluster",
            ...
            with Finally("I drop the user"):
                node.query(
                    f"DROP ROLE IF EXISTS {alter_role_name} ON CLUSTER sharded_cluster"
                )

...
def privilege_granted_directly_or_via_role(self, node=None):
    ...
    with Suite("user with direct privilege"):
        with user(node, user_name):
            with When(
                f"I run checks that {user_name} is only able to execute DETACH TABLE with required privileges"
            ):
                privilege_check(
    ...
    with Suite("user with privilege via role"):
        with user(node, user_name), role(node, role_name):
            ...
            with And(
                f"I run checks that {user_name} with {role_name} is only able to execute DETACH TABLE with required privileges"
            ):
                privilege_check(

...
def privilege_granted_directly_or_via_role(self, node=None):
    ...
    with Suite("user with direct privilege"):
        with user(node, user_name):
            with When(
                f"I run checks that {user_name} is only able to execute CREATE DATABASE with required privileges"
            ):
                privilege_check(
    ...
    with Suite("user with privilege via role"):
        with user(node, user_name), role(node, role_name):
            ...
            with And(
                f"I run checks that {user_name} with {role_name} is only able to execute CREATE DATABASE with required privileges"
            ):
                privilege_check(
        ...
        try:
            ...
            with Then("I attempt to attach a database"):
                node.query(
                    ...
                    settings=[("user", user_name)],
                    exitcode=80,
                    message="DB::Exception: Received from localhost:9000. DB::Exception: Database engine must be specified for ATTACH DATABASE query",
                )

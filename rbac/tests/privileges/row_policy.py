...
@TestStep(Given)
def create_table(self, table_name, node=None):
...
@TestStep(Given)
def insert_data(self, table_name, node=None):
    """Insert simple data."""
    if node is None:
        node = self.context.cluster.node("clickhouse1")
...
@TestStep(Given)
def create_row_policy(self, table_name, node=None):
    ...
    try:
        node.query(
            f"CREATE ROW POLICY pol0 ON default.{table_name} FOR SELECT USING x >= 0 TO default"
        )
        yield
...
@TestScenario
@Requirements(RQ_SRS_006_RBAC_RowPolicy_MultiIf("1.0"))
def multiIf_alias(self, node=None):
    ...
    with When(
        "I select with a query where the multiIf alias matches the name of a column."
    ):
        output_dup = node.query(
            f"""
            WITH Test AS (
            SELECT
...
@TestScenario
@Requirements(RQ_SRS_006_RBAC_RowPolicy_Columns("1.0"))
...
@TestScenario
@Requirements(RQ_SRS_006_RBAC_RowPolicy_Rows("1.0"))
def row_policy_filtering(self, node=None):
    ...
    try:
        ...
        with And(
            "I have a row policy that filters a row that should already be filtered by WITH."
        ):
            node.query(
...
@TestScenario
@Requirements(RQ_SRS_006_RBAC_RowPolicy_MultiIfLength("1.0"))
...
@TestScenario
@Requirements(RQ_SRS_006_RBAC_RowPolicy_Parts("1.0"))
...
@TestFeature
@Name("row policy")
@Requirements(RQ_SRS_006_RBAC_RowPolicy("1.0"))

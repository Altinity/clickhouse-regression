        ...
        Suite(
            ...
            examples=Examples(
                ...
                [
                    ...
                    for row in create_role.examples
                ],
    ...
    with Scenario("CREATE ROLE without privilege"):
        ...
        try:
            ...
            with Then("I check the user can't create a role"):
                node.query(
                    f"CREATE ROLE {create_role_name}",
        ...
        try:
            ...
            with Then("I check the user can create a role"):
                if check_clickhouse_version(">=24.4")(self) and privilege != "ALL":
                    node.query(
                        f"CREATE ROLE {create_role_name} ON CLUSTER sharded_cluster",
                        settings=[("user", f"{user_name}")],
                        exitcode=exitcode,
                        message=message,
                    )
            ...
            with And("I add CLUSTER privilege and check the user can create a role"):
                if check_clickhouse_version(">=24.4")(self) and privilege != "ALL":
            ...
            with Finally("I drop the role"):
                node.query(
                    f"DROP ROLE IF EXISTS {create_role_name} ON CLUSTER sharded_cluster"
                )

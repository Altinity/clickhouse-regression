...
def clickhouse_connection(
    host, database="default", user="default", port=8443, password=""
):
    conn = connect(
        host=host,
        user=user,
...
def convert_to_clickhouse_table(user_name, table_name, source):
    ...
    if primary_key is None:
        logging.warning(
            "No PK found for " + table_name + " defaulting to order by tuple()"
        )
        primary_key = "tuple()"
...
def get_unix_timezone_from_mysql_timezone(timezone):
    ...
    for tz in timezones:
        offset = (
            datetime.datetime.now(zoneinfo.ZoneInfo(tz)).utcoffset().total_seconds()
            / 60
            / 60
        )
        ...
        timezone_from_offset += (
            f"{abs(offset_int):02}:{abs(round((offset-offset_int)*60)):02}"
        )
...
def load_schema(args, dry_run=False):
    ...
    with get_connection(args) as conn:
        ...
        database_file = (
            args.dump_dir + f"/{args.mysql_source_database}-schema-create.sql.gz"
        )
...
def get_column_list(
    schema_map, schema, table, virtual_columns, transform=False, mysqlshell=False
):
    key = f"{schema}.{table}"
    column_list = "*"
...
def load_data_mysqlshell(args, timezone, schema_map, dry_run=False):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.threads) as executor:
        ...
        for file in glob.glob(schema_files):
            ...
            ) + glob.glob(dfile + f"{schema}@{table_name}.tsv.zst")
            columns = get_column_list(
                schema_map,
                schema,
                table_name,
                args.virtual_columns,
                transform=False,
                mysqlshell=args.mysqlshell,
            )
            transformed_columns = get_column_list(
                schema_map,
                schema,
            ...
            for data_file in data_files:
                # double quote escape logic https://github.com/ClickHouse/ClickHouse/issues/10624
                structure = (
                    columns.replace(",", " Nullable(String),") + " Nullable(String)"
                )
...
def main():
   ...
   """
    ...
    parser.add_argument("--clickhouse_password", help="CH password", required=True)
    parser.add_argument(
        "--clickhouse_database", help="Clickhouse database name", required=True
    )
    parser.add_argument(
        "--mysql_source_database", help="MySQL source schema", required=True
    )
    parser.add_argument("--dump_dir", help="Location of dump files", required=True)
    parser.add_argument(
        "--threads", type=int, default=8, help="Number of threads", required=True
    )
    parser.add_argument("--debug", dest="debug", action="store_true", default=False)
    parser.add_argument(
        "--schema_only", dest="schema_only", action="store_true", default=False
    )
    parser.add_argument(
        "--data_only", dest="data_only", action="store_true", default=False
    )
    parser.add_argument("--dry_run", dest="dry_run", action="store_true", default=False)
    parser.add_argument(
        "--virtual_columns",
        help="virtual_columns",
        nargs="+",
        default=["`_sign`", "`_version`"],
    )
    parser.add_argument(
        "--mysqlshell",
        help="using a util.dumpSchemas",
        dest="mysqlshell",
        action="store_true",
        default=False,
    )
    ...
    assert args.mysqlshell and check_program_exists(
        "zstd"
    ), "zstd should be in the PATH for util.dumpSchemas load"

...
def prewhere_with_final_clause(self, table, node=None):
    """Execute select 'PREWHERE' query step with `FINAL` clause and with --final setting disabled."""
    if node is None:
        node = self.context.cluster.node("clickhouse1")

    with When(f"I make `SELECT PREWHERE FINAL` from table {table.name}"):
        node.query(
...
@TestStep
@Name("SELECT PREWHERE with --final")
def prewhere_with_force_final(self, table, node=None):
    """Execute select 'PREWHERE' query step without `FINAL` clause but with --final setting enabled."""
    if node is None:
        node = self.context.cluster.node("clickhouse1")

    with When(
        f"I make `SELECT PREWHERE` with --final setting enabled from table {table.name}"
    ):
        node.query(
            f"SELECT * FROM {table.name} "
            f" PREWHERE x > 3 ORDER BY (id, x, someCol) FORMAT JSONEachRow;",
...
def prewhere_with_final_clause_and_force_final(
    ...
    with When(
        f"I make `SELECT PREWHERE FINAL` with --final setting enabled from table {table.name}"
    ):
        node.query(
            f"SELECT * FROM {table.name} {'FINAL' if table.final_modifier_available else ''}"
            f" PREWHERE x > 3 ORDER BY (id, x, someCol) FORMAT JSONEachRow;",
...
def prewhere_result_check(self, table, node=None):
...
def prewhere_negative_result_check(self, table, node=None):

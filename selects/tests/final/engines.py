...
@TestOutline
def select_final(self, ignore_force_select_final=False, select_count=None, engine=None, graphite=False):
    ...
    try:
        ...
        with Then(f"I make insert into table"):
            if graphite:
                node.query(f"insert into {table_name} values (now(),'a',1,'b',1,'c', 1,1,1,1)")
                node.query(f"insert into {table_name} values (now(),'a',1,'b',1,'c', 1,1,1,1)")
                pause()
            else:
...
def force_select_final_available_engines(self):
    ...
    """
    for engine in ["ReplacingMergeTree()", "CollapsingMergeTree(sign)", "SummingMergeTree()", "AggregatingMergeTree()"]:
        with Given(f"I check 'force_select_final' and 'ignore_force_select_final' settings for {engine} table ENGINE"):
            select_final(
                ignore_force_select_final=False, select_count=1, engine=engine
            )
            select_final(
...
def force_select_final_unavailable_engines(self):
    ...
    MergeTree table engines.
    """
    for engine in ["MergeTree()",
                   "VersionedCollapsingMergeTree(sign,version)"]:
        with Given(f"I check 'force_select_final' and 'ignore_force_select_final' settings for {engine} table ENGINE"):
            if engine == "GraphiteMergeTree('graphite_rollup_example')":
                select_final(
                    ignore_force_select_final=False, select_count=2, engine=engine, graphite=True
                )
                select_final(
                    ignore_force_select_final=True, select_count=2, engine=engine, graphite=True
                )
            else:
                select_final(

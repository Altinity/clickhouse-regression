...
def select_final(self, ignore_force_select_final=False, select_count=None, engine=None):
    ...
    try:
        with Given(
            ...
            node.query(f"create table if not exists {table_name} (x String, sign Int8 DEFAULT 1)"
...
def force_select_final_replacingmergetree(self):
    ...
    """
    select_final(ignore_force_select_final=False, select_count=1, engine="ReplacingMergeTree")
    select_final(ignore_force_select_final=True, select_count=2,  engine="ReplacingMergeTree")
...
def force_select_final_collapsingmergetree(self):
    ...
    """
    select_final(ignore_force_select_final=False, select_count=1, engine="CollapsingMergeTree")
    select_final(ignore_force_select_final=True, select_count=2,  engine="CollapsingMergeTree")


@TestFeature
@Requirements(RQ_SRS_032_ClickHouse_AutomaticFinalModifier_SupportedTableEngines_MergeTree("1.0"))
@Name("engines")
def feature(self):

"""Helpers for running the selects suite with content-addressed default storage."""
...
<clickhouse>
    ...
    This file is filled in by selects/cas_mode.py when the suite is started with
...
def enable_cas_default_storage(self, s3_cache=False):
    ...
    cas_config_path().write_text(
        cas_storage_config("selects-cas-{replica}", with_s3_cache=s3_cache)
    )
...
def check_cas_mode(test):
    """True when selects was started with ``--cas`` or ``--cas-s3-cache``."""

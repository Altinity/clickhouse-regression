...
def check_replica_path_intersection(self):
    ...
    with Given("I create table on cluster"):
        ...
        create_table(
            ...
            engine=f"ReplicatedMergeTree('/clickhouse/tables/replicated_cluster/default/{second_table_name}',"+"'{replica}')",
    ...
    with And("I rename table"):
        self.context.node_1.query(f"RENAME TABLE {first_table_name} TO {second_table_name}")
    ...
    with And("I insert data"):
        self.context.node_2.query(f"INSERT INTO {second_table_name} SELECT * FROM numbers(200000000)")

    with And("I insert into first table"):
        self.context.node_2.query(
            f"INSERT INTO {first_table_name} SELECT * FROM numbers(200000000)"
        )

...
@TestScenario
def struct_type(self, minio_root_user, minio_root_password):
    ...
    with When(f"define schema and create {namespace}.{table_name} table"):
        nf1, next_id = generate_iceberg_nested_field(
            "details1", max_width=10, max_depth=20, start_id=2
        )
        nf2, _ = generate_iceberg_nested_field(
            "details2", max_width=10, max_depth=20, start_id=next_id
        )
        schema = Schema(
            NestedField(
    ...
    with And(f"insert data into {namespace}.{table_name} table"):
        field1 = generate_arrow_field(nf1)
        field2 = generate_arrow_field(nf2)
        data_rows = []
        for symbol in ["AAPL", "GOOG"]:
            data1 = generate_data_from_nested_field(nf1)
            data2 = generate_data_from_nested_field(nf2)
            row_dict = {"string": symbol}
            row_dict.update(data1)
            row_dict.update(data2)
            data_rows.append(row_dict)

        final_arrow_schema = pa.schema([("string", pa.string()), field1, field2])
        df = pa.Table.from_pylist(data_rows, schema=final_arrow_schema)
        table.append(df)

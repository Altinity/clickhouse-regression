...
def array_product_alias(self):
    ...
    alias_columns = [
        {
            "name": "array_product",
            "expression": "arrayProduct(array_col)",
            "hybrid_type": "Float64",
        },
    ]
    watermark = {
...
@TestScenario
def array_product_alias_in_watermark(self):
    """
    Define parameters for test case and call main outline.
    Test alias: array_product ALIAS arrayProduct(array_col)
    ...
    watermark = {
        "left_predicate": "array_product >= 1000.0",
        "right_predicate": "array_product < 1000.0",
    }
    expected = {"exitcode": 0, "error_message": None}
    test_queries = [
        "SELECT id, value, date_col FROM {hybrid_table} ORDER BY id",
        "SELECT array_product FROM {hybrid_table} ORDER BY id",
        "SELECT id, value, array_product FROM {hybrid_table} ORDER BY id",
        "SELECT id, value, array_product FROM {hybrid_table} WHERE array_product >= 1000.0 ORDER BY id",
    ]
    order_by = "(date_col, id)"
    partition_by = "toYYYYMM(date_col)"
...
def feature(self, minio_root_user=None, minio_root_password=None):
    ...
    Scenario(run=array_product_alias_in_watermark)

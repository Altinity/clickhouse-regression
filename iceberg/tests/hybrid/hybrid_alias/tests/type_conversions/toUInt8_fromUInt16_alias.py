...
def toUInt8_fromUInt16_alias(self):
    ...
    alias_columns = [
        {
            "name": "value_uint8",
            "expression": "toUInt8(small_value)",
            "hybrid_type": "UInt8",
        },
    ]
    watermark = {
...
@TestScenario
def toUInt8_fromUInt16_alias_in_watermark(self):
    """
    Define parameters for test case and call main outline.
    Test alias: value_uint8 ALIAS toUInt8(small_value) - Shrinking conversion from UInt16 to UInt8
    ...
    watermark = {
        "left_predicate": "value_uint8 >= 50",
        "right_predicate": "value_uint8 < 50",
    }
    expected = {"exitcode": 0, "error_message": None}
    test_queries = [
        "SELECT id, small_value, date_col FROM {hybrid_table} ORDER BY id",
        "SELECT value_uint8 FROM {hybrid_table} ORDER BY id",
        "SELECT id, small_value, value_uint8 FROM {hybrid_table} ORDER BY id",
        "SELECT id, small_value, value_uint8 FROM {hybrid_table} WHERE value_uint8 >= 50 ORDER BY id",
    ]
    order_by = "(date_col, id)"
    partition_by = "toYYYYMM(date_col)"
...
def feature(self, minio_root_user=None, minio_root_password=None):
    ...
    Scenario(run=toUInt8_fromUInt16_alias_in_watermark)

...
@TestScenario
def toUInt32_alias_in_watermark(self):
    """
    Define parameters for test case and call main outline.
    Test alias: value_uint32 ALIAS toUInt32(value)
    ...
    watermark = {
        "left_predicate": "value_uint32 >= 5000",
        "right_predicate": "value_uint32 < 5000",
    }
    expected = {"exitcode": 0, "error_message": None}
    test_queries = [
        "SELECT id, value, date_col FROM {hybrid_table} ORDER BY id",
        "SELECT value_uint32 FROM {hybrid_table} ORDER BY id",
        "SELECT id, value, value_uint32 FROM {hybrid_table} ORDER BY id",
        "SELECT id, value, value_uint32 FROM {hybrid_table} WHERE value_uint32 >= 5000 ORDER BY id",
    ]
    order_by = "(date_col, id)"
    partition_by = "toYYYYMM(date_col)"
...
def feature(self, minio_root_user=None, minio_root_password=None):
    ...
    Scenario(run=toUInt32_alias_in_watermark)

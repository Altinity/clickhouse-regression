...
@TestScenario
def toHour_alias_in_watermark(self):
    """
    Define parameters for test case and call main outline.
    Test alias: hour ALIAS toHour(datetime_col)
    ...
    watermark = {"left_predicate": "hour >= 12", "right_predicate": "hour < 12"}
    expected = {"exitcode": 0, "error_message": None}
    test_queries = [
        "SELECT id, value, date_col FROM {hybrid_table} ORDER BY id",
        "SELECT hour FROM {hybrid_table} ORDER BY id",
        "SELECT id, value, hour FROM {hybrid_table} ORDER BY id",
        "SELECT id, value, hour FROM {hybrid_table} WHERE hour >= 12 ORDER BY id",
    ]
    order_by = "(date_col, id)"
    partition_by = "toYYYYMM(date_col)"
...
def feature(self, minio_root_user=None, minio_root_password=None):
    ...
    Scenario(run=toHour_alias_in_watermark)

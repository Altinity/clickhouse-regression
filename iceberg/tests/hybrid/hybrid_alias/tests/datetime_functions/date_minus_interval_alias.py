...
@TestScenario
def date_minus_interval_alias_in_watermark(self):
    """
    Define parameters for test case and call main outline.
    Test alias: date_minus_interval ALIAS date_col - INTERVAL 1 MONTH
    ...
    watermark = {
        "left_predicate": "date_minus_interval >= '2010-01-01'",
        "right_predicate": "date_minus_interval < '2010-01-01'",
    }
    expected = {"exitcode": 0, "error_message": None}
    test_queries = [
        "SELECT id, value, date_col FROM {hybrid_table} ORDER BY id",
        "SELECT date_minus_interval FROM {hybrid_table} ORDER BY id",
        "SELECT id, value, date_minus_interval FROM {hybrid_table} ORDER BY id",
        "SELECT id, value, date_minus_interval FROM {hybrid_table} WHERE date_minus_interval >= '2010-01-01' ORDER BY id",
    ]
    order_by = "(date_col, id)"
    partition_by = "toYYYYMM(date_col)"
...
def feature(self, minio_root_user=None, minio_root_password=None):
    ...
    Scenario(run=date_minus_interval_alias_in_watermark)

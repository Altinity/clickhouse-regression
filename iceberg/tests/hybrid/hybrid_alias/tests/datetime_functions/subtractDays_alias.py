    ...
    alias_columns = [
        {
            ...
            "expression": "subtractDays(date_col, 7)",
            "hybrid_type": "Date",
        },
    ]
    watermark = {
...
@TestScenario
def subtractDays_alias_in_watermark(self):
    """
    Define parameters for test case and call main outline.
    Test alias: date_minus_days ALIAS subtractDays(date_col, 7)
    ...
    watermark = {
        "left_predicate": "date_minus_days >= '2010-01-01'",
        "right_predicate": "date_minus_days < '2010-01-01'",
    }
    expected = {"exitcode": 0, "error_message": None}
    test_queries = [
        "SELECT id, value, date_col FROM {hybrid_table} ORDER BY id",
        "SELECT date_minus_days FROM {hybrid_table} ORDER BY id",
        "SELECT id, value, date_minus_days FROM {hybrid_table} ORDER BY id",
        "SELECT id, value, date_minus_days FROM {hybrid_table} WHERE date_minus_days >= '2010-01-01' ORDER BY id",
    ]
    order_by = "(date_col, id)"
    partition_by = "toYYYYMM(date_col)"
...
def feature(self, minio_root_user=None, minio_root_password=None):
    ...
    Scenario(run=subtractDays_alias_in_watermark)
